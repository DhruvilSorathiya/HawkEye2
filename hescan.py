...
from hawkeye import HawkEyeDb, HawkEyeEvent, HawkEyePlugin, HawkEyeTarget, HawkEyeHelpers, HawkEyeThreadPool, HawkEyeCorrelator, logger


def startHawkEyeScanner(loggingQueue, *args, **kwargs):
    logger.logWorkerSetup(loggingQueue)
    return HawkEyeScanner(*args, **kwargs)


class HawkEyeScanner():
    """HawkEyeScanner object.
    ...
    def __init__(self, scanName: str, scanId: str, targetValue: str, targetType: str, moduleList: list, globalOpts: dict, start: bool = True) -> None:
        ...
        self.__dbh = HawkEyeDb(self.__config)
        ...
        self.__he = HawkEye(self.__config)
        self.__he.dbh = self.__dbh
        ...
        # Create our target
        try:
            self.__target = HawkEyeTarget(self.__targetValue, self.__targetType)
        except (TypeError, ValueError) as e:
            self.__he.status(f"Scan [{self.__scanId}] failed: {e}")
        ...
        self.__dbh.scanConfigSet(self.__scanId, self.__he.configSerialize(deepcopy(self.__config)))
        ...
        if proxy_type:
            ...
            if proxy_type == '4':
                ...
                self.__he.status(f"Scan [{self.__scanId}] failed: Invalid proxy type: {proxy_type}")
                self.__setStatus("ERROR-FAILED", None, time.time() * 1000)
            ...
            if not proxy_host:
                self.__he.status(f"Scan [{self.__scanId}] failed: Proxy type is set ({proxy_type}) but proxy address value is blank")
                self.__setStatus("ERROR-FAILED", None, time.time() * 1000)
            ...
            self.__he.debug(f"Using proxy: {proxy}")
            self.__he.socksProxy = proxy
        else:
            self.__he.socksProxy = None
        ...
        # Set the user agent
        self.__config['_useragent'] = self.__he.optValueToData(self.__config['_useragent'])

        # Set up the Internet TLD list.
        # If the cached does not exist or has expired, reload it from scratch.
        tld_data = self.__he.cacheGet("internet_tlds", self.__config['_internettlds_cache'])
        if tld_data is None:
            tld_data = self.__he.optValueToData(self.__config['_internettlds'])
            if tld_data is None:
                self.__he.status(f"Scan [{self.__scanId}] failed: Could not update TLD list")
                self.__setStatus("ERROR-FAILED", None, time.time() * 1000)
                raise ValueError("Could not update TLD list")
            self.__he.cachePut("internet_tlds", tld_data)

        self.__config['_internettlds'] = tld_data.splitlines()

        self.__setStatus("INITIALIZING", time.time() * 1000, None)

        self.__sharedThreadPool = HawkEyeThreadPool(threads=self.__config.get("_maxthreads", 3), name='sharedThreadPool')
    ...
    def __startScan(self) -> None:
        ...
        try:
            ...
            self.__he.status(f"Scan [{self.__scanId}] for '{self.__target.targetValue}' initiated.")
            ...
            self.__he.debug(f"Loading {len(self.__moduleList)} modules ...")
            for modName in self.__moduleList:
                ...
                if modName not in self.__config['__modules__']:
                    self.__he.error(f"Failed to load module: {modName}")
                    continue
                ...
                try:
                    ...
                    self.__he.error(f"Module {modName} initialization failed", exc_info=True)
                    continue
                ...
                try:
                    ...
                    mod.setup(self.__he, self.__modconfig[modName])
                except Exception:
                    self.__he.error(f"Module {modName} initialization failed", exc_info=True)
                    mod.errorState = True
                    continue
                ...
                if self.__config['_socks1type'] != '':
                    try:
                        ...
                        self.__he.error(f"Module {modName} socket setup failed: {e}")
                        continue
                ...
                if self.__config['__outputfilter']:
                    try:
                        ...
                        self.__he.error(f"Module {modName} output filter setup failed: {e}")
                        continue
                ...
                try:
                    ...
                    self.__he.error(f"Module {modName} target enrichment failed: {e}")
                    continue
                ...
                try:
                    ...
                    self.__he.error(f"Module {modName} failed to set target '{self.__target}': {e}")
                    continue
                ...
                try:
                    ...
                    self.__he.error(f"Module {modName} event queue setup failed: {e}")
                    continue

                self.__moduleInstances[modName] = mod
                self.__he.status(f"{modName} module loaded.")

            self.__he.debug(f"Scan [{self.__scanId}] loaded {len(self.__moduleInstances)} modules.")
            ...
            rootEvent = HawkEyeEvent("ROOT", self.__targetValue, "", None)
            psMod.notifyListeners(rootEvent)
            firstEvent = HawkEyeEvent(self.__targetType, self.__targetValue,
                                         "HawkEye UI", rootEvent)
            psMod.notifyListeners(firstEvent)

            # Special case.. check if an INTERNET_NAME is also a domain
            if self.__targetType == 'INTERNET_NAME' and self.__he.isDomain(self.__targetValue, self.__config['_internettlds']):
                firstEvent = HawkEyeEvent('DOMAIN_NAME', self.__targetValue, "HawkEye UI", rootEvent)
                psMod.notifyListeners(firstEvent)
            ...
            self.__he.status(f"Scan [{self.__scanId}] aborted.")
            ...
            if not failed:
                ...
                self.__he.status(f"Scan [{self.__scanId}] completed.")
            self.__dbh.close()

    def runCorrelations(self) -> None:
        """Run correlation rules."""

        self.__he.status(f"Running {len(self.__config['__correlationrules__'])} correlation rules on scan {self.__scanId}.")
        ruleset = dict()
        for rule in self.__config['__correlationrules__']:
            ruleset[rule['id']] = rule['rawYaml']
        corr = HawkEyeCorrelator(self.__dbh, ruleset, self.__scanId)
    ...
    def waitForThreads(self) -> None:
        ...
        try:
            ...
            while True:
                ...
                try:
                    heEvent = self.eventQueue.get_nowait()
                    self.__he.debug(f"waitForThreads() got event, {heEvent.eventType}, from eventQueue.")
                except queue.Empty:
                    # check if we're finished
                    if self.threadheinished(log_status):
                        sleep(.1)
                        # but are we really?
                        if self.threadheinished(log_status):
                            if final_passes < 1:
                            ...
                            sleep(.1)
                            while not self.threadheinished(log_status):
                ...
                if not isinstance(heEvent, HawkEyeEvent):
                    raise TypeError(f"heEvent is {type(heEvent)}; expected HawkEyeEvent")
                ...
                for mod in self.__moduleInstances.values():
                    ...
                    if not mod.errorState and mod.incomingEventQueue is not None:
                        ...
                        if heEvent.eventType in watchedEvents or "*" in watchedEvents:
                            mod.incomingEventQueue.put(deepcopy(heEvent))
    ...
    def threadheinished(self, log_status: bool = False) -> bool:
        ...
        for mod in self.__moduleInstances.values():
            if mod.errorState and mod.incomingEventQueue is not None:
                self.__he.debug(f"Clearing and unsetting incomingEventQueue for errored module {mod.__name__}.")
                with suppress(Exception):
                    while 1:
        ...
        if not modules_running and not queues_empty:
            self.__he.debug("Clearing queues for stalled/aborted modules.")
            for mod in self.__moduleInstances.values():
                with suppress(Exception):
                    while True:
                        mod.incomingEventQueue.get_nowait()
        ...
        if log_status:
            ...
            if not events_queued:
                events_queued = 'None'
            self.__he.debug(f"Events queued: {sum([m[-1] for m in modules_waiting]):,} ({events_queued})")
            if modules_running:
                self.__he.debug(f"Modules running: {len(modules_running):,} ({', '.join(modules_running)})")
            if modules_errored:
                self.__he.debug(f"Modules errored: {len(modules_errored):,} ({', '.join(modules_errored)})")
