...
from hawkeye import HawkEyeCorrelator
...
from auth.models import db  # <-- IMPORTANT: import db from models.py

app = Flask(__name__)
app.config['SECRET_KEY'] = 'your-secret-key'
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///spiderfoot.db'
app.config['MAIL_SERVER'] = 'smtp.yourserver.com'
app.config['MAIL_PORT'] = 587
app.config['MAIL_USE_TLS'] = True
app.config['MAIL_USERNAME'] = 'you@example.com'
app.config['MAIL_PASSWORD'] = 'yourpassword'
app.config['MAIL_DEFAULT_SENDER'] = 'spiderfoot@example.com'
...
def main() -> None:
    ...
    heConfig = {
        ...
        '__database': f"{HawkEyeHelpers.dataPath()}/hawkeye.db",
    ...
    args = None
    p = argparse.ArgumentParser(description=f"HawkEye {__version__}: Open Source Intelligence Automation.")
    ...
    p.add_argument("-V", "--version", action='store_true', help="Display the version of HawkEye and exit.")
    p.add_argument("-max-threads", type=int, help="Max number of modules to run concurrently.")
    args = p.parse_args()

    if args.version:
        print(f"HawkEye {__version__}: Open Source Intelligence Automation.")
        sys.exit(0)

    if args.max_threads:
        heConfig['_maxthreads'] = args.max_threads

    if args.debug:
        heConfig['_debug'] = True
    else:
        heConfig['_debug'] = False

    if args.q:
        heConfig['__logging'] = False

    loggingQueue = mp.Queue()
    logListenerSetup(loggingQueue, heConfig)
    logWorkerSetup(loggingQueue)
    log = logging.getLogger(f"hawkeye.{__name__}")

    # Add descriptions of the global config options
    heConfig['__globaloptdescs__'] = heOptdescs

    # Load each module in the modules directory with a .py extension
    try:
        mod_dir = os.path.dirname(os.path.abspath(__file__)) + '/modules/'
        heModules = HawkEyeHelpers.loadModulesAsDict(mod_dir, ['hep_template.py'])
    ...
    try:
        ...
        correlationRulesRaw = HawkEyeHelpers.loadCorrelationRulesRaw(correlations_dir, ['template.yaml'])
    except BaseException as e:
    ...
    try:
        dbh = HawkEyeDb(heConfig)
    except Exception as e:
        log.critical(f"Failed to initialize database: {e}", exc_info=True)
        sys.exit(-1)

    # Sanity-check the rules and parse them
    heCorrelationRules = list()
    if not correlationRulesRaw:
        log.error(f"No correlation rules found in correlations directory: {correlations_dir}")
    else:
        try:
            correlator = HawkEyeCorrelator(dbh, correlationRulesRaw)
            heCorrelationRules = correlator.get_ruleset()
        except Exception as e:
            log.critical(f"Failure initializing correlation rules: {e}", exc_info=True)
            sys.exit(-1)

    # Add modules and correlation rules to heConfig so they can be used elsewhere
    heConfig['__modules__'] = heModules
    heConfig['__correlationrules__'] = heCorrelationRules
    ...
    if args.correlate:
        ...
        try:
            ...
            corr = HawkEyeCorrelator(dbh, correlationRulesRaw, args.correlate)
    ...
    if args.modules:
        ...
        for m in sorted(heModules.keys()):
            if "__" in m:
                continue
            print(f"{m.ljust(25)}  {heModules[m]['descr']}")
        sys.exit(0)

    if args.types:
        dbh = HawkEyeDb(heConfig, init=True)
    ...
    if args.l:
        ...
        heWebUiConfig['host'] = host
        heWebUiConfig['port'] = port

        start_web_server(heWebUiConfig, heConfig, loggingQueue)
        sys.exit(0)

    start_scan(heConfig, heModules, args, loggingQueue)


def start_scan(heConfig: dict, heModules: dict, args, loggingQueue) -> None:
    """Start scan

    Args:
        heConfig (dict): HawkEye config options
        heModules (dict): modules
        args (argparse.Namespace): command line args
        loggingQueue (Queue): main HawkEye logging queue
    """
    log = logging.getLogger(f"hawkeye.{__name__}")
    ...
    if "." not in target and not target.startswith("+") and '"' not in target:
        target = f"\"{target}\""
    targetType = HawkEyeHelpers.targetTypeFromString(target)
    ...
    if args.t:
        types = args.t
        modlist = he.modulesProducing(types)
    ...
    if args.u:
        ...
        for mod in heConfig['__modules__']:
            if usecase == 'All' or usecase in heConfig['__modules__'][mod]['group']:
                modlist.append(mod)

    # Add hep__stor_stdout to the module list
    typedata = dbh.eventTypes()
    types = dict()
    for r in typedata:
        types[r[1]] = r[0]

    hep__stor_stdout_opts = heConfig['__modules__']['hep__stor_stdout']['opts']
    hep__stor_stdout_opts['_eventtypes'] = types
    if args.f:
        if args.f and not args.t:
            log.error("You can only use -f with -t. Use --help for guidance.")
            sys.exit(-1)
        hep__stor_stdout_opts['_showonlyrequested'] = True
    if args.F:
        hep__stor_stdout_opts['_requested'] = args.F.split(",")
        hep__stor_stdout_opts['_showonlyrequested'] = True
    if args.o:
        if args.o not in ["tab", "csv", "json"]:
            log.error("Invalid output format selected. Must be 'tab', 'csv' or 'json'.")
            sys.exit(-1)
        hep__stor_stdout_opts['_format'] = args.o
    if args.t:
        hep__stor_stdout_opts['_requested'] = args.t.split(",")
    if args.n:
        hep__stor_stdout_opts['_stripnewline'] = True
    if args.r:
        hep__stor_stdout_opts['_showsource'] = True
    if args.S:
        hep__stor_stdout_opts['_maxlength'] = args.S
    if args.D:
        hep__stor_stdout_opts['_csvdelim'] = args.D
    if args.x:
        tmodlist = list()
        modlist = list()
        xmods = he.modulesConsuming([targetType])
        for mod in xmods:
            if mod not in modlist:
                tmodlist.append(mod)
        ...
        for mod in tmodlist:
            for r in rtypes:
                if not heModules[mod]['provides']:
                    continue
                if r in heModules[mod].get('provides', []) and mod not in modlist:
                    modlist.append(mod)
    ...
    modlist += ["hep__stor_db", "hep__stor_stdout"]

    if heConfig['__logging']:
        log.info(f"Modules enabled ({len(modlist)}): {','.join(modlist)}")

    cfg = he.configUnserialize(dbh.configGet(), heConfig)
    ...
    try:
        p = mp.Process(target=startHawkEyeScanner, args=(loggingQueue, scanName, scanId, target, targetType, modlist, cfg))
        p.daemon = True
        p.start()
    except BaseException as e:
...
def start_web_server(heWebUiConfig: dict, heConfig: dict, loggingQueue=None) -> None:
    """Start the web server so you can start looking at results

    Args:
        heWebUiConfig (dict): web server options
        heConfig (dict): HawkEye config options
        loggingQueue (Queue): main HawkEye logging queue
    """
    log = logging.getLogger(f"hawkeye.{__name__}")

    web_host = heWebUiConfig.get('host', '127.0.0.1')
    web_port = heWebUiConfig.get('port', 5001)
    web_root = heWebUiConfig.get('root', '/')
    cors_origins = heWebUiConfig.get('cors_origins', [])
    ...
    conf = {
        ...
        '/static': {
            ...
            'tools.staticdir.root': f"{os.path.dirname(os.path.abspath(__file__))}/HawkEye"
        }
    }

    secrets = dict()
    passwd_file = HawkEyeHelpers.dataPath() + '/passwd'
    ...
    #     warn_msg += "Refer to https://www.hawkeye.net/documentation/#security.\n"
    #     warn_msg += "********************************************************************\n"
    #     log.warning(warn_msg)

    using_ssl = False
    key_path = HawkEyeHelpers.dataPath() + '/hawkeye.key'
    crt_path = HawkEyeHelpers.dataPath() + '/hawkeye.crt'
    ...
    print("")
    print("*************************************************************")
    print(" Use HawkEye by starting your web browser of choice and ")
    ...
    cherrypy.quickstart(HawkEyeWebUi(heWebUiConfig, heConfig, loggingQueue), script_name=web_root, config=conf)
...
if __name__ == '__main__':
    ...
    if sys.version_info < (3, 7):
        print("HawkEye requires Python 3.7 or higher.")
        sys.exit(-1)

    # 2. Legacy file checks
    if os.path.exists('hawkeye.db'):
        print(f"ERROR: HawkEye.db file exists in {os.path.dirname(__file__)}")
        print("HawkEye no longer supports loading the HawkEye.db database from the application directory.")
        print(f"The database is now loaded from your home directory: {Path.home()}/.hawkeye/hawkeye.db")
        print(f"This message will go away once you move or remove HawkEye.db from {os.path.dirname(__file__)}")
        sys.exit(-1)

    if os.path.exists('passwd'):
        print(f"ERROR: passwd file exists in {os.path.dirname(__file__)}")
        print("HawkEye no longer supports loading credentials from the application directory.")
        print(f"The passwd file is now loaded from your home directory: {Path.home()}/.hawkeye/passwd")
        print(f"This message will go away once you move or remove passwd from {os.path.dirname(__file__)}")
        sys.exit(-1)

    # 3. Choose mode: CLI/CherryPy or Flask Auth
    if len(sys.argv) > 1:
        main()
    else:
