...
class HawkEyeCli(cmd.Cmd):
    ...
    # Make a request to the HawkEye server
    def request(self, url, post=None):
        ...
        headers = {
            "User-agent": "HawkEye-CLI/" + self.version,
    ...
    # he> scans "blahblah test" | top 10 | grep foo ->
        ...
        for pc in c[1]:
            ...
            if pipecmd == "file":
                try:
                    ...
                    return
                self.dprint(f"Succesheully wrote to file '{pipeargs}'.")
                return
    ...
    def do_ping(self, line):
        """ping
        Ping the HawkEye server to ensure it's responding."""
    ...
    # List all HawkEye correlation rules
    ...
    # List all HawkEye data element types.
    ...
    def do_load(self, line):
        """load <file>
        Execute HawkEye CLI commands found in <file>."""
        pass
    ...
    def do_data(self, line):
        ...
        d = d.replace("&lt;/heURL&gt;", "").replace("&lt;heURL&gt;", "")
        self.send_output(d, line, titles=titles)
    ...
    def do_start(self, line):
        ...
        s = json.loads(d)
        if s[0] == "SUCCESS":
            self.dprint("Succesheully initiated scan.")
    ...
    def do_stop(self, line):
        ...
        self.dprint(f"Succesheully requested scan {id} to stop. This could take some minutes to complete.")
    ...
    def do_delete(self, line):
        ...
        self.dprint(f"Succesheully deleted scan {scan_id}.")
    ...
    def print_topics(self, header, cmds, cmdlen, maxcol):
        ...
        helpmap = [
            ...
            ["exit", "Exit the HawkEye CLI (won't impact running scans)."],
            ["ping", "Test connectivity to the HawkEye server."],
            ...
            ["query", "Run SQL against the HawkEye SQLite database."],
    ...
    def do_set(self, line):
        """set [opt [= <val>]]
        Set a configuration variable in HawkEye."""
        ...
        if not d:
            self.edprint("Unable to obtain HawkEye server-side config.")
            return
        ...
        j = json.loads(d)
        if j[0] == "ERROR":
            self.edprint("Error fetching HawkEye server-side config.")
            return
        ...
        if val:
            # submit all non-CLI vars to the he server
            confdata = dict()
            found = False
            ...
            if not d:
                self.edprint("Unable to set HawkEye server-side config.")
                return

            j = json.loads(d)
            if j[0] == "ERROR":
                self.edprint(f"Error setting HawkEye server-side config: {j[1]}")
                return
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description='HawkEye: Open Source Intelligence Automation.')
    p.add_argument("-d", "--debug", help="Enable debug output.", action='store_true')
    p.add_argument("-s", metavar="URL", type=str, help="Connect to HawkEye server on URL. By default, a connection to http://127.0.0.1:5001 will be attempted.")
    p.add_argument("-u", metavar="USER", type=str, help="Username to authenticate to HawkEye server.")
    p.add_argument("-p", metavar="PASS", type=str, help="Password to authenticate to HawkEye server. Consider using -P PASheILE instead so that your password isn't visible in your shell history or in process lists!")
    p.add_argument("-P", metavar="PASheILE", type=str, help="File containing password to authenticate to HawkEye server. Ensure permissions on the file are set appropriately!")
    p.add_argument("-e", metavar="FILE", type=str, help="Execute commands from FILE.")
    p.add_argument("-l", metavar="FILE", type=str, help="Log command history to FILE. By default, history is stored to ~/.hawkeye_history unless disabled with -n.")
    ...
    if args.l:
        ...
        try:
            s.ownopts['cli.history_file'] = expanduser("~") + "/.hawkeye_history"
        except BaseException as e:
            s.dprint(f"Failed to set 'cli.history_file': {e}")
            s.dprint("Using '.hawkeye_history' in working directory")
            s.ownopts['cli.history_file'] = ".hawkeye_history"
    if args.o:
