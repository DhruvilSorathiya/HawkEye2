...
from .threadpool import HawkEyeThreadPool
...
class HawkEyePluginLogger(logging.Logger):
    """Used only in HawkEyePlugin to prevent modules
...
class HawkEyePlugin():
    ...
    Attributes:
        ...
        _currentEvent (HawkEyeEvent): Current event being processed
        _currentTarget (str): Target currently being acted against
        _name_: Name of this module, set at startup time
        __hedb__: Direct handle to the database - not to be directly used
                  by modules except the hep__stor_db module.
    ...
    __scanId__ = None
    # (only used in HawkEye HX) tracking of data sources
    __dataSource__ = None
    ...
    # HawkEye object, set in each module's setup() function
    he = None
    ...
    def log(self):
        if self._log is None:
            logging.setLoggerClass(HawkEyePluginLogger)  # temporarily set logger class
            self._log = logging.getLogger(f"hawkeye.{self.__name__}")  # init HawkEyePluginLogger
    ...
    def setup(self, he, userOpts: dict = {}) -> None:
        """Will always be overriden by the implementer.

        Args:
            he (HawkEye): HawkEye object
            userOpts (dict): TBD
        """
        pass
    ...
    def setTarget(self, target) -> None:
        ...
        Args:
            target (HawkEyeTarget): target

        Raises:
            TypeError: target argument was invalid type
        """
        from hawkeye import HawkEyeTarget

        if not isinstance(target, HawkEyeTarget):
            raise TypeError(f"target is {type(target)}; expected HawkEyeTarget")

        self._currentTarget = target

    def setDbh(self, dbh) -> None:
        """Used to set the database handle, which is only to be used
        by modules in very rare/exceptional cases (e.g. hep__stor_db)

        Args:
            dbh (HawkEyeDb): database handle
        """
        self.__hedb__ = dbh
    ...
    def tempStorage(self) -> dict:
        ...
        Note:
            Required for HawkEye HX compatibility of modules.
    ...
    def notifyListeners(self, heEvent) -> None:
        ...
        Args:
            heEvent (HawkEyeEvent): event

        Raises:
            TypeError: heEvent argument was invalid type
        """
        ...
        eventName = heEvent.eventType
        eventData = heEvent.data
        ...
        # is what's linked to children. For instance, hep_dns may find
        # xyz.abc.com, and then hep_ripe obtains some raw data for the
        # same, and then hep_dns finds xyz.abc.com in there, we should
        # suppress the notification of that to other modules, as the
        # original xyz.abc.com notification from hep_dns will trigger
        ...
        prevEvent = heEvent.sourceEvent
        while prevEvent is not None:
            if prevEvent.sourceEvent is not None and prevEvent.sourceEvent.eventType == heEvent.eventType and prevEvent.sourceEvent.data.lower() == eventData.lower():
                storeOnly = True
                break
        ...
        if self.outgoingEventQueue is not None:
            self.outgoingEventQueue.put(heEvent)
        # otherwise, call other modules directly
            ...
            for listener in self._listenerModules:
                ...
                listener._currentEvent = heEvent
                ...
                try:
                    listener.handleEvent(heEvent)
                except Exception as e:
                    self.he.error(f"Module ({listener.__module__}) encountered an error: {e}")
                    # set errorState
    ...
    def checkForStop(self) -> bool:
        ...
        scanstatus = self.__hedb__.scanInstanceGet(self.__scanId__)
    ...
    def handleEvent(self, heEvent) -> None:
    ...
    def threadWorker(self) -> None:
        try:
            ...
            self.setDbh(HawkEyeDb(self.opts))
            self.he._dbh = self.__hedb__

            if not (self.incomingEventQueue and self.outgoingEventQueue):
                self.he.error("Please set up queues before starting module as thread")
                return

            while not self.checkForStop():
                try:
                    heEvent = self.incomingEventQueue.get_nowait()
                except queue.Empty:
                    sleep(.3)
                    continue
                if heEvent == 'FINISHED':
                    self.he.debug(f"{self.__name__}.threadWorker() got \"FINISHED\" from incomingEventQueue.")
                    self.poolExecute(self.finish)
                else:
                    self.he.debug(f"{self.__name__}.threadWorker() got event, {heEvent.eventType}, from incomingEventQueue.")
                    self.poolExecute(self.handleEvent, heEvent)
        except KeyboardInterrupt:
            self.he.debug(f"Interrupted module {self.__name__}.")
            self._stopScanning = True
        except Exception as e:
            import traceback
            self.he.error(f"Exception ({e.__class__.__name__}) in module {self.__name__}."
                          + traceback.format_exc())
            # set errorState
            self.he.debug(f"Setting errorState for module {self.__name__}.")
            self.errorState = True
            # clear incoming queue
            if self.incomingEventQueue:
                self.he.debug(f"Emptying incomingEventQueue for module {self.__name__}.")
                with suppress(queue.Empty):
                    while 1:
                        self.incomingEventQueue.get_nowait()
    ...
    def poolExecute(self, callback, *args, **kwargs) -> None:
        ...
        """
        if self.__name__.startswith('hep__stor_'):
