...
from hawkeye import HawkEyeDb, HawkEyeHelpers


class HawkEyeSqliteLogHandler(logging.Handler):
    ...
    def makeDbh(self) -> None:
        """TBD."""
        self.dbh = HawkEyeDb(self.opts)


def logListenerSetup(loggingQueue, opts: dict = None) -> 'logging.handlers.QueueListener':
    """Create and start a HawkEye log listener in its own thread.
    ...
    Args:
        ...
        opts (dict): HawkEye config

    Returns:
        hawkEyeLogListener (logging.handlers.QueueListener): Log listener
    """
    if opts is None:
        opts = dict()
    ...
    log_dir = HawkEyeHelpers.logPath()
    debug_handler = logging.handlers.TimedRotatingFileHandler(
        f"{log_dir}/hawkeye.debug.log",
        when="d",
        interval=1,
        backupCount=30
    )
    ...
    if doLogging and opts is not None:
        sqlite_handler = HawkEyeSqliteLogHandler(opts)
        sqlite_handler.setLevel(logLevel)
        sqlite_handler.setFormatter(log_format)
        handlers.append(sqlite_handler)
    hawkEyeLogListener = QueueListener(loggingQueue, *handlers)
    hawkEyeLogListener.start()
    atexit.register(stop_listener, hawkEyeLogListener)
    return hawkEyeLogListener
