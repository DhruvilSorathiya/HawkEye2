...
class HawkEyeEvent():
    """HawkEyeEvent object representing identified data and associated meta data.
    ...
    Attributes:
        ...
        sourceEvent (HawkEyeEvent): HawkEyeEvent that triggered this event
        sourceEventHash (str): Hash of the HawkEyeEvent event that triggered this event
    ...
    def __init__(self, eventType: str, data: str, module: str, sourceEvent: 'HawkEyeEvent') -> None:
        """Initialize HawkEye event object.

        Args:
            eventType (str): Event type, e.g. URL_FORM, RAW_DATA, etc.
            data (str): Event data, e.g. a URL, port number, webpage content, etc.
            module (str): Module from which the event originated
            sourceEvent (HawkEyeEvent): HawkEyeEvent event that triggered this event
        """
    ...
    @property
    def sourceEvent(self) -> 'HawkEyeEvent':
    ...
    @sourceEvent.setter
    def sourceEvent(self, sourceEvent: 'HawkEyeEvent') -> None:
        """Source event which lead to this event.

        Args:
            sourceEvent (HawkEyeEvent): source event
        ...
        if not isinstance(sourceEvent, HawkEyeEvent):
            raise TypeError(f"sourceEvent is {type(sourceEvent)}; expected HawkEyeEvent()")
