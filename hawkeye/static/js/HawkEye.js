//-------------------------------------------------------------------------------
// Name:         hawkeye.js
// Purpose:      All the javascript code for the hawkeye aspects of the UI.
//
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  themeToggler.addEventListener("click", () => {
    ...
    if (localStorage.getItem("theme") === "dark-theme") {
      ...
      link.rel = "stylesheet";
      link.type = "text/css";
      link.href = "${docroot}/static/css/hawkeye.css";
...
he.replace_heurltag = function (data) {
  if (data.toLowerCase().indexOf("&lt;heurl&gt;") >= 0) {
    data = data.replace(
      RegExp("&lt;heurl&gt;(.*)&lt;/heurl&gt;", "img"),
...
he.remove_heurltag = function (data) {
  if (data.toLowerCase().indexOf("&lt;heurl&gt;") >= 0) {
    data = data
      .toLowerCase()
      .replace("&lt;heurl&gt;", "")
  ...
  if (data.toLowerCase().indexOf("<heurl>") >= 0) {
    data = data.toLowerCase().replace("<heurl>", "").replace("</heurl>", "");
  }
  return data;
};

he.search = function (scan_id, value, type, postFunc) {
  he.fetchData(
    docroot + "/search",
...
he.deleteScan = function(scan_id, callback) {
    var req = $.ajax({
      type: "GET",
    ...
    req.fail(function (hr, textStatus, errorThrown) {
        ...
        he.log("Error deleting scans: " + scan_id + ": " + hr.responseText);
    });
};

he.stopScan = function(scan_id, callback) {
    var req = $.ajax({
      type: "GET",
    ...
    req.done(function() {
        ...
        he.log("Aborted scans: " + scan_id);
        callback();
    });
    req.fail(function (hr, textStatus, errorThrown) {
        alertify.error('<i class="glyphicon glyphicon-minus-sign"></i> <b>Error</b><br/><br/>' + hr.responseText);
        he.log("Error stopping scans: " + scan_id + ": " + hr.responseText);
    });
};

he.fetchData = function (url, postData, postFunc) {
  var req = $.ajax({
    type: "POST",
    url: url,
    data: postData,
...
/*
he.simpleTable = function(id, data, cols, linkcol=null, linkstring=null, sortable=true, rowfunc=null) {
