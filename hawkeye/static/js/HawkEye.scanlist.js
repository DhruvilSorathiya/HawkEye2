...
function rerunSelected() {
    ...
    he.log("Re-running scans: " + ids.join(','));
...
function exportSelected(type) {
    ...
    if (!ids) {
        he.log("Error: no scan(s) selected");
        return;
    }
    ...
    switch(type) {
        case "gexf":
            he.log("Exporting scans as " + type + ": " + ids.join(','));
        ...
        case "json":
            ...
            break;
        default:
            he.log("Error: Invalid export type: " + type);
    }
    $("#loader").fadeOut(500);
}
...
function showlist(types, filter) {
    globalTypes = types;
    globalFilter = filter;
    he.fetchData(docroot + '/scanlist', null, function(data) {
        if (data.length == 0) {
            $("#loader").fadeOut(500);
...
function showlisttable(types, filter, data) {
   ...
   buttons += "<button rel='tooltip' data-toggle='dropdown' data-title='Export Selected' id='btn-export' class='btn btn-default btn-success dropdown-toggle download-button'><i class='glyphicon glyphicon-download-alt glyphicon-white'></i></button>";
    buttons += "<ul class='dropdown-menu'>";
    buttons += "<li><a href=''>CSV</a></li>";
    buttons += "<li><a href=''>Excel</a></li>";
    buttons += "<li><a href=''>GEXF</a></li>";
    buttons += "<li><a href=''>JSON</a></li>";
    buttons += "</ul>";
    buttons += "</div>";
