...
function he_viz_countTailNodes(arg) {
    var data = arg;
    var count = 0;
    ...
    for (var i = 0; i < data.length; i++) {
        for (var p in data[i]) {
            ...
            if (p == "children" && data[i].children != null) {
                count += he_viz_countTailNodes(data[i].children);
            }
        }
    }
...
function he_viz_countTotalNodes(arg) {
    var data = arg;
    var count = 0;
    ...
    for (var i = 0; i < data.length; i++) {
        for (var p in data[i]) {
            ...
            if (p == "children" && data[i].children != null) {
                count += he_viz_countTotalNodes(data[i].children);
            }
        }
    }
...
function he_viz_countLevels(arg, levelsDeep, maxLevels) {
    var data = arg;
    ...
    for (var i = 0; i < data.length; i++) {
        for (var p in data[i]) {
            ...
            if (p == "children" && data[i].children != null) {
                levels++;
                arr = he_viz_countLevels(data[i].children, levels, max);
                levels = arr[0];
                max = arr[1];
            }
...
function he_viz_vbar(targetId, gdata) {
...
*/
 ...
 //   svg.call(tip);
    ...
    for (i = 0; i < gdata.length; i++) {
        data[i] = he_viz_vbar_type(gdata[i])
    }
...
function he_viz_vbar_type(d) {
      d.pct = +d.pct;
      return d;
}

function he_viz_dendrogram(targetId, data) {
    var plotData = data['tree'];
    var dataMap = data['data'];
    var width = he_viz_countLevels([plotData], 0, 0)[1] * 170;
    var height = he_viz_countTailNodes([plotData]) * 20;
    ...
    node.append("text")
        ...
        .text(function(d) { 
            if (dataMap[d.name][1].length > 20) {
                return he.remove_heurltag(dataMap[d.name][1].substring(0, 20) + "...");
            } else {
                return he.remove_heurltag(dataMap[d.name][1]);
            }
        });
    ...
    function buildPopupMessage(data) {
        ...
        message += "<tr><td><b>Data:</b></td><td><pre>" + he.remove_heurltag(data[1])
        message += "</pre></td></tr>";
        message += "</table>";
        return message;
    }
...
function he_viz_bubble(targetId, plotData) { 
    var diameter = 900,
        format = d3.format(",d"),
