...
class HawkEyeThreadPool:
    ...
    Example 1: using map()
        with HawkEyeThreadPool(self.opts["_maxthreads"]) as pool:
            # callback("a", "arg1"), callback("b", "arg1"), ...
            for result in pool.map(
                    callback,
                    ["a", "b", "c", "d"],
                    "arg1",
                    taskName="hep_testmodule"
                    saveResult=True
                ):
                yield result
    ...
    Example 2: using submit()
        with HawkEyeThreadPool(self.opts["_maxthreads"]) as pool:
            ...
            pool.submit(callback, "arg1", taskName="hep_testmodule", saveResult=True)
            pool.submit(callback, "arg2", taskName="hep_testmodule", saveResult=True)
            for result in pool.shutdown()["hep_testmodule"]:
                yield result
    """

    def __init__(self, threads: int = 100, qsize: int = 10, name: str = '') -> None:
        """Initialize the HawkEyeThreadPool class.
