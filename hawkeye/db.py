...
class HawkEyeDb:
    ...
    # Queries for creating the HawkEye database
    createSchemaQueries = [
    ...
    eventDetails = [
        ['ROOT', 'Internal HawkEye Root event', 1, 'INTERNAL'],
        ...
        with self.dbhLock:
            try:
                ...
                try:
                    ...
                    raise IOError("Tried to set up the HawkEye database schema, but failed") from e
            ...
            try:
                ...
                try:
                    ...
                    raise IOError("Looks like you are running a pre-4.0 database. Unfortunately "
                                  "HawkEye wasn't able to migrate you, so you'll need to delete "
                                  "your HawkEye database in order to proceed.") from None
    ...
    def scanEventStore(self, instanceId: str, heEvent, truncateSize: int = 0) -> None:
        """Store an event in the database.

        Args:
            instanceId (str): scan instance ID
            heEvent (HawkEyeEvent): event to be stored in the database
        ...
        Raises:
            ...
            IOError: database I/O failed
        """
        from hawkeye import HawkEyeEvent
        ...
        if not isinstance(heEvent, HawkEyeEvent):
            raise TypeError(f"heEvent is {type(heEvent)}; expected HawkEyeEvent()") from None

        if not isinstance(heEvent.generated, float):
            raise TypeError(f"heEvent.generated is {type(heEvent.generated)}; expected float()") from None

        if not heEvent.generated:
            raise ValueError("heEvent.generated is empty") from None

        if not isinstance(heEvent.eventType, str):
            raise TypeError(f"heEvent.eventType is {type(heEvent.eventType,)}; expected str()") from None

        if not heEvent.eventType:
            raise ValueError("heEvent.eventType is empty") from None

        if not isinstance(heEvent.data, str):
            raise TypeError(f"heEvent.data is {type(heEvent.data)}; expected str()") from None

        if not heEvent.data:
            raise ValueError("heEvent.data is empty") from None

        if not isinstance(heEvent.module, str):
            raise TypeError(f"heEvent.module is {type(heEvent.module)}; expected str()") from None

        if not heEvent.module and heEvent.eventType != "ROOT":
            raise ValueError("heEvent.module is empty") from None

        if not isinstance(heEvent.confidence, int):
            raise TypeError(f"heEvent.confidence is {type(heEvent.confidence)}; expected int()") from None

        if not 0 <= heEvent.confidence <= 100:
            raise ValueError(f"heEvent.confidence value is {type(heEvent.confidence)}; expected 0 - 100") from None

        if not isinstance(heEvent.visibility, int):
            raise TypeError(f"heEvent.visibility is {type(heEvent.visibility)}; expected int()") from None

        if not 0 <= heEvent.visibility <= 100:
            raise ValueError(f"heEvent.visibility value is {type(heEvent.visibility)}; expected 0 - 100") from None

        if not isinstance(heEvent.risk, int):
            raise TypeError(f"heEvent.risk is {type(heEvent.risk)}; expected int()") from None

        if not 0 <= heEvent.risk <= 100:
            raise ValueError(f"heEvent.risk value is {type(heEvent.risk)}; expected 0 - 100") from None

        if not isinstance(heEvent.sourceEvent, HawkEyeEvent) and heEvent.eventType != "ROOT":
            raise TypeError(f"heEvent.sourceEvent is {type(heEvent.sourceEvent)}; expected str()") from None

        if not isinstance(heEvent.sourceEventHash, str):
            raise TypeError(f"heEvent.sourceEventHash is {type(heEvent.sourceEventHash)}; expected str()") from None

        if not heEvent.sourceEventHash:
            raise ValueError("heEvent.sourceEventHash is empty") from None

        storeData = heEvent.data
        ...
        qvals = [instanceId, heEvent.hash, heEvent.eventType, heEvent.generated,
                 heEvent.confidence, heEvent.visibility, heEvent.risk,
                 heEvent.module, storeData, heEvent.sourceEventHash]
