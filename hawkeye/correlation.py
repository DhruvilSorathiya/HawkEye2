...
class HawkEyeCorrelator:
    """HawkEye correlation capabilities.

    Todo:
        Make the rule checking per analysis method
    """

    log = logging.getLogger("hawkeye.correlator")
    dbh = None
    scanId = None
    types = None
    rules = list()
    ...
    def __init__(self, dbh: HawkEyeDb, ruleset: dict, scanId: str = None) -> None:
        """Initialize HawkEye correlator engine with scan ID and ruleset.
        ...
        if not isinstance(dbh, HawkEyeDb):
            raise TypeError(f"dbh is {type(dbh)}; expected HawkEyeDb()")
    ...
    def build_db_criteria(self, matchrule: dict) -> dict:
        ...
        Returns:
            dict: criteria to be used with HawkEyeDb.scanResultEvent()
