...
class hep_fsecure_riddler(HawkEyePlugin):
    ...
    def login(self):
        ...
        res = self.he.fetchUrl('https://riddler.io/auth/login',
    ...
    def query(self, qry):
        ...
        res = self.he.fetchUrl('https://riddler.io/api/search',
                               postData=json.dumps(params),
                               headers=headers,
                               useragent=self.opts['_useragent'],
                               timeout=self.opts['_fetchtimeout'])
    ...
    def handleEvent(self, event):
        ...
        if srcModuleName == 'hep_fsecure_riddler':
        ...
        if self.opts['username'] == '' or self.opts['password'] == '':
            self.error('You enabled hep_fsecure_riddler but did not set an API username/password!')
            self.errorState = True
            return
        ...
        e = HawkEyeEvent('RAW_RIR_DATA', str(data), self.__name__, event)
        ...
        for addr in set(addrs):
            if self.he.validIP(addr):
                evt = HawkEyeEvent('IP_ADDRESS', addr, self.__name__, event)
                self.notifyListeners(evt)

        for coord in set(coords):
            evt = HawkEyeEvent('PHYSICAL_COORDINATES', coord, self.__name__, event)
            self.notifyListeners(evt)

# End of hep_fsecure_riddler class
