# -*- coding: utf-8 -*-
# -------------------------------------------------------------------------------
# Name:        hep_tool_onesixtyone
# Purpose:     HawkEye plug-in for using the onesixtyone tool.
...
class hep_tool_onesixtyone(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if srcModuleName == "hep_tool_onesixtyone":
            self.debug("Skipping event from myself.")
            return

        if not self.opts['onesixtyone_path']:
            self.error("You enabled hep_tool_onesixtyone but did not set a path to the tool!")
            self.errorState = True
            return
        ...
        for target in targets:
            ...
            for result in stdout.split("\n"):
                ...
                if target != eventData:
                    srcevent = HawkEyeEvent("IP_ADDRESS", target, self.__name__, event)
                    self.notifyListeners(srcevent)

                e = HawkEyeEvent('UDP_PORT_OPEN', f"{target}:161", self.__name__, srcevent)
                self.notifyListeners(e)

                e = HawkEyeEvent("UDP_PORT_OPEN_INFO", result, self.__name__, e)
                self.notifyListeners(e)

# End of hep_tool_onesixtyone class
