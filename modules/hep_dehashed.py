...
class hep_dehashed(HawkEyePlugin):
    ...
    def query(self, event, per_page, start):
        ...
        res = self.he.fetchUrl(queryString,
                               headers=headers,
                               timeout=15,
        ...
        if res['code'] == "400":
            ...
            time.sleep(5)
            res = self.he.fetchUrl(queryString, headers=headers, timeout=15, useragent=self.opts['_useragent'], verify=True)
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "" or self.opts['api_key_username'] == "":
            self.error("You enabled hep_dehashed but did not set an API key/API Key Username!")
            self.errorState = True
            return
        ...
        while currentPage <= maxPages:
            ...
            for row in data.get('entries'):
                ...
                if eventName == "EMAILADDR":
                    if email == eventData:
                        evt = HawkEyeEvent('EMAILADDR_COMPROMISED', f"{email} [{leakSource}]", self.__name__, event)
                        self.notifyListeners(evt)

                        if password:
                            evt = HawkEyeEvent('PASSWORD_COMPROMISED', f"{email}:{password} [{leakSource}]", self.__name__, event)
                            self.notifyListeners(evt)

                        if passwordHash:
                            evt = HawkEyeEvent('HASH_COMPROMISED', f"{email}:{passwordHash} [{leakSource}]", self.__name__, event)
                            self.notifyListeners(evt)

                        evt = HawkEyeEvent('RAW_RIR_DATA', str(row), self.__name__, event)
                        self.notifyListeners(evt)

                if eventName == "DOMAIN_NAME":
                    pevent = HawkEyeEvent("EMAILADDR", email, self.__name__, event)
                    if email not in emailResults:
                        self.notifyListeners(pevent)
                        emailResults.add(email)

                    evt = HawkEyeEvent('EMAILADDR_COMPROMISED', f"{email} [{leakSource}]", self.__name__, pevent)
                    self.notifyListeners(evt)

                    if password:
                        evt = HawkEyeEvent('PASSWORD_COMPROMISED', f"{email}:{password} [{leakSource}]", self.__name__, pevent)
                        self.notifyListeners(evt)

                    if passwordHash:
                        evt = HawkEyeEvent('HASH_COMPROMISED', f"{email}:{passwordHash} [{leakSource}]", self.__name__, pevent)
                        self.notifyListeners(evt)

                    evt = HawkEyeEvent('RAW_RIR_DATA', str(row), self.__name__, pevent)
                    self.notifyListeners(evt)
