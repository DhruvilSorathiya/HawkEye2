# -*- coding: utf-8 -*-
# -------------------------------------------------------------------------------
# Name:        hep_sociallinks
# Purpose:     Hawkeye plugin to query SocialLinks.io to gather intelligence
...
class hep_sociallinks(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "":
            self.error("You enabled hep_sociallinks but did not set an API key!")
            self.errorState = True
            return
        ...
        if eventName == "PHONE_NUMBER":
            ...
            if resultSet:
                if resultSet.get('first_name') and resultSet.get('last_name'):
                    evt = HawkEyeEvent("HUMAN_NAME", f"{resultSet.get('first_name')} {resultSet.get('last_name')}", self.__name__, event)
                    self.notifyListeners(evt)
                if resultSet.get('username'):
                    evt = HawkEyeEvent("USERNAME", resultSet.get('username'), self.__name__, event)
                    self.notifyListeners(evt)
            ...
            if resultSet:
                ...
                if resultSet.get('phone_number'):
                    evt = HawkEyeEvent("PHONE_NUMBER", resultSet.get('phone_number'), self.__name__, event)
                    self.notifyListeners(evt)
            ...
            if data is None:
                ...
                if resultSet:
                    ...
                    if resultSet.get('profileurl').get('_content'):
                        evt = HawkEyeEvent("SOCIAL_MEDIA", f"Flickr: <HEURL>{resultSet.get('profileurl').get('_content')}</HEURL>", self.__name__, event)
                        self.notifyListeners(evt)
            ...
            if data is None:
                ...
                if resultSet:
                    ...
                    if resultSet.get('skypeId'):
                        evt = HawkEyeEvent("ACCOUNT_EXTERNAL_OWNED", f"Skype [{resultSet.get('skypeId')}]", self.__name__, event)
                        self.notifyListeners(evt)
                        evt = HawkEyeEvent("USERNAME", resultSet.get('skypeId'), self.__name__, event)
                        self.notifyListeners(evt)
            ...
            if data is None:
                ...
                if resultSet:
                    ...
                    if resultSet.get('companyName'):
                        evt = HawkEyeEvent("COMPANY_NAME", resultSet.get('companyName'), self.__name__, event)
                        self.notifyListeners(evt)
                    if resultSet.get('headline'):
                        evt = HawkEyeEvent("JOB_TITLE", resultSet.get('headline'), self.__name__, event)
                        self.notifyListeners(evt)

                    evt = HawkEyeEvent("SOCIAL_MEDIA", f"LinkedIn: <HEURL>{resultSet.get('linkedInUrl')}</HEURL>", self.__name__, event)
                    self.notifyListeners(evt)
            ...
            for humanName in humanNames:
                evt = HawkEyeEvent("HUMAN_NAME", humanName, self.__name__, event)
                self.notifyListeners(evt)

            for geoInfo in geoInfos:
                evt = HawkEyeEvent("GEOINFO", geoInfo, self.__name__, event)
                self.notifyListeners(evt)
