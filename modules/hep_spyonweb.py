...
# -------------------------------------------------------------------------------
# Name:         hep_spyonweb
# Purpose:      HawkEye plug-in to search SpyOnWeb for hosts sharing the
...
class hep_spyonweb(HawkEyePlugin):
    ...
    def query(self, endpoint, qry, limit=100):
        ...
        res = self.he.fetchUrl(url, timeout=self.opts['timeout'], useragent=self.opts['_useragent'])
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "":
            self.error("You enabled hep_spyonweb but did not set an API key!")
        ...
        if eventName in ['INTERNET_NAME', 'DOMAIN_NAME']:
            ...
            if google_adsense:
                for r in list(google_adsense.keys()):
                    evt = HawkEyeEvent("WEB_ANALYTICS_ID", f"Google AdSense: {r}", self.__name__, event)
            ...
            if google_analytics:
                for r in list(google_analytics.keys()):
                    evt = HawkEyeEvent("WEB_ANALYTICS_ID", f"Google Analytics: {r}", self.__name__, event)
                    self.notifyListeners(evt)
        ...
        if eventName in ['WEB_ANALYTICS_ID']:
            ...
            for r in list(data.keys()):
                ...
                evt = HawkEyeEvent("AFFILIATE_INTERNET_NAME", r, self.__name__, event)
                self.notifyListeners(evt)

                if self.he.isDomain(r, self.opts['_internettlds']):
                    evt = HawkEyeEvent("AFFILIATE_DOMAIN_NAME", r, self.__name__, event)
                    self.notifyListeners(evt)
