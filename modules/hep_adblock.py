...
class hep_adblock(HawkEyePlugin):
    ...
    def retrieveBlocklist(self, blocklist_url):
        ...
        blocklist = self.he.cacheGet(f"adblock_{blocklist_url}", 24)

        if blocklist is not None:
            return self.setBlocklistRules(blocklist)

        res = self.he.fetchUrl(blocklist_url, timeout=30)
        ...
        self.he.cachePut(f"adblock_{blocklist_url}", res['content'])
        ...
        try:
            if eventName == 'PROVIDER_JAVASCRIPT':
                if self.rules and self.rules.should_block(eventData, {'third-party': True, 'script': True}):
                    evt = HawkEyeEvent("URL_ADBLOCKED_EXTERNAL", eventData, self.__name__, event)
            ...
            if eventName == 'LINKED_URL_INTERNAL':
                if self.rules and self.rules.should_block(eventData):
                    evt = HawkEyeEvent("URL_ADBLOCKED_INTERNAL", eventData, self.__name__, event)
                    self.notifyListeners(evt)
