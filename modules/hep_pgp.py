...
# -------------------------------------------------------------------------------
# Name:         hep_pgp
# Purpose:      HawkEye plug-in for looking up e-mail addresses in PGP
...
class hep_pgp(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if eventName == "EMAILADDR" and self.opts['retrieve_keys']:
            ...
            keys = HawkEyeHelpers.extractPgpKeysFromText(res['content'])
            self.info(f"Found {len(keys)} public PGP keys")

            for key in keys:
                self.debug(f"Found public key: {key}")
                evt = HawkEyeEvent("PGP_KEY", key, self.__name__, event)
                self.notifyListeners(evt)

# End of hep_pgp class
