...
# -------------------------------------------------------------------------------
# Name:         hep_trashpanda
# Purpose:      Hawkeye plugin to query Trashpanda - got-hacked.wtf API to gather intelligence about
...
class hep_trashpanda(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key_username'] == "" or self.opts['api_key_password'] == "":
            self.error("You enabled hep_trashpanda but did not set an API username / password!")
        ...
        leaksiteUrls = set()
        for row in data:
            evt = HawkEyeEvent("PASSWORD_COMPROMISED", f"{row.get('email')}:{row.get('password')} [{row.get('paste')}]", self.__name__, event)
        ...
        for leaksiteUrl in leaksiteUrls:
            try:
                ...
                res = self.he.fetchUrl(leaksiteUrl, timeout=self.opts['_fetchtimeout'],
                                       useragent=self.opts['_useragent'])
                ...
                evt = HawkEyeEvent("LEAKSITE_URL", leaksiteUrl, self.__name__, event)
                self.notifyListeners(evt)

                evt = HawkEyeEvent("LEAKSITE_CONTENT", res['content'], self.__name__, evt)
                self.notifyListeners(evt)
            except Exception as e:
                self.debug(f"Error while fetching leaksite content : {str(e)}")

# End of hep_trashpanda class
