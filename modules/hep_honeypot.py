...
# -------------------------------------------------------------------------------
# Name:        hep_honeypot
# Purpose:     HawkEye plug-in for looking up whether IPs appear in the
...
class hep_honeypot(HawkEyePlugin):
    ...
    def queryAddr(self, qaddr, parentEvent):
        ...
        try:
            ...
            addrs = self.he.resolveHost(lookup)
            if not addrs:
                return
        ...
        evt = HawkEyeEvent(malicious_type, f"ProjectHoneyPot ({qaddr}): {text}\n<HEURL>{url}</HEURL>", self.__name__, parentEvent)
        self.notifyListeners(evt)

        evt = HawkEyeEvent(blacklist_type, f"ProjectHoneyPot ({qaddr}): {text}\n<HEURL>{url}</HEURL>", self.__name__, parentEvent)
        self.notifyListeners(evt)
