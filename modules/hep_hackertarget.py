# -*- coding: utf-8 -*-
# -------------------------------------------------------------------------------
# Name:         hep_hackertarget
# Purpose:      HawkEye plug-in to search HackerTarget.com for hosts sharing
...
class hep_hackertarget(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if srcModuleName == "hep_hackertarget" and eventName == "IP_ADDRESS":
        ...
        if eventName == 'DOMAIN_NAME_PARENT':
            ...
            evt = HawkEyeEvent('RAW_DNS_RECORDS', "\n".join(records), self.__name__, event)
                ...
                for host in set(hosts):
                    ...
                    if self.he.isDomain(host, self.opts['_internettlds']):
                        if evt_type.startswith('AFFILIATE'):
                            evt = HawkEyeEvent('AFFILIATE_DOMAIN_NAME', host, self.__name__, event)
                            self.notifyListeners(evt)
                        else:
        ...
        for ip in qrylist:
            ...
            for h in hosts:
                ...
                if h not in myres and h != ip:
                    if self.opts['verify'] and not self.he.validateIP(h, ip):
                    ...
                    if self.cohostcount < self.opts['maxcohost']:
                        ...
                        # link to the co-host.
                        if eventName == "NETBLOCK_OWNER":
                            ipe = HawkEyeEvent("IP_ADDRESS", ip, self.__name__, event)
                            self.notifyListeners(ipe)
                            evt = HawkEyeEvent("CO_HOSTED_SITE", h.lower(), self.__name__, ipe)
                            self.notifyListeners(evt)
                        else:
                            evt = HawkEyeEvent("CO_HOSTED_SITE", h.lower(), self.__name__, event)
                            self.notifyListeners(evt)
            ...
            if eventName == 'NETBLOCK_OWNER':
                pevent = HawkEyeEvent("IP_ADDRESS", ip, self.__name__, event)
            ...
            if self.opts.get('http_headers', True):
                ...
                if http_headers is not None:
                    e = HawkEyeEvent('WEBSERVER_HTTPHEADERS', json.dumps(http_headers), self.__name__, pevent)
                    e.actualSource = ip
                    self.notifyListeners(e)

# End of hep_hackertarget class
