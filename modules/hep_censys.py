...
class hep_censys(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        for addr in qrylist:
            ...
            if eventName == 'NETBLOCK_OWNER':
                ...
                pevent = HawkEyeEvent("IPV6_ADDRESS", addr, self.__name__, event)
                self.notifyListeners(pevent)
            else:
                pevent = event

            e = HawkEyeEvent("RAW_RIR_DATA", json.dumps(rec), self.__name__, pevent)
            ...
            try:
                ...
                if location:
                    ...
                    if geoinfo:
                        e = HawkEyeEvent("GEOINFO", geoinfo, self.__name__, pevent)
                        self.notifyListeners(e)
            except Exception as e:
            ...
            try:
                ...
                if services:
                    ...
                    for service in services:
                        ...
                        if port:
                            ...
                            if transport_protocol == "UDP":
                                evt = HawkEyeEvent("UDP_PORT_OPEN", f"{addr}:{port}", self.__name__, pevent)
                                self.notifyListeners(evt)
                            elif transport_protocol == "TCP":
                                evt = HawkEyeEvent("TCP_PORT_OPEN", f"{addr}:{port}", self.__name__, pevent)
                                self.notifyListeners(evt)
                                if banner:
                    ...
                    for software in set(softwares):
                        evt = HawkEyeEvent("SOFTWARE_USED", software, self.__name__, pevent)
                        self.notifyListeners(evt)

                    for banner in set(tcp_banners):
                        evt = HawkEyeEvent("TCP_PORT_OPEN_BANNER", str(banner), self.__name__, pevent)
                        self.notifyListeners(evt)
            except Exception as e:
            ...
            try:
                ...
                if autonomous_system:
                    ...
                    if asn:
                        e = HawkEyeEvent("BGP_AS_MEMBER", str(asn), self.__name__, pevent)
                        self.notifyListeners(e)

                    bgp_prefix = autonomous_system.get('bgp_prefix')
                    if bgp_prefix and self.he.validIpNetwork(bgp_prefix):
                        if ':' in bgp_prefix:
                            e = HawkEyeEvent("NETBLOCKV6_MEMBER", str(bgp_prefix), self.__name__, pevent)
                        else:
                            e = HawkEyeEvent("NETBLOCK_MEMBER", str(bgp_prefix), self.__name__, pevent)
                        self.notifyListeners(e)
            except Exception as e:
            ...
            try:
                ...
                if operating_system:
                    ...
                    if os:
                        e = HawkEyeEvent("OPERATING_SYSTEM", os, self.__name__, pevent)
                        self.notifyListeners(e)
            except Exception as e:
                self.error(f"Error encountered processing operating_system record for {addr}: {e}")

# End of hep_censys class
