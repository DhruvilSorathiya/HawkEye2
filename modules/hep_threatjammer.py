...
class hep_threatjammer(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if ip_info is None:
            self.he.error(f"Error processing JSON response for {eventData} from ThreatJammer.com")
            return

        score = ip_info.get('score')
        if not score:
            self.he.error(f"No risk score found for {eventData} from ThreatJammer.com. Skipping.")
            return
        risk_score = int(score)

        risk = ip_info.get("risk")
        if not risk:
            self.he.error(f"No risk type found for {eventData} from ThreatJammer.com. Skipping.")
            return
