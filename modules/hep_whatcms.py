# -------------------------------------------------------------------------------
# Name:        hep_whatcms
# Purpose:     HawkEye plug-in to check which web technology is used
...
class hep_whatcms(HawkEyePlugin):
    ...
    def queryCmsDetect(self, qry):
        ...
        res = self.he.fetchUrl('https://whatcms.org/APIEndpoint/Detect?' + urllib.parse.urlencode(params),
    ...
    def queryCmsTechnology(self, qry):
        ...
        res = self.he.fetchUrl('https://whatcms.org/APIEndpoint/Technology?' + urllib.parse.urlencode(params),
                               timeout=self.opts['timeout'],
                               useragent=self.opts['_useragent'])
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == '':
            self.error('You enabled hep_whatcms but did not set an API key!')
            self.errorState = True
            return
        ...
        for result in results:
            if result.get('name'):
                ...
                evt = HawkEyeEvent('WEBSERVER_TECHNOLOGY', software, self.__name__, event)
                self.notifyListeners(evt)
            else:
                self.debug('No web technology found for ' + eventData)

# End of hep_whatcms class
