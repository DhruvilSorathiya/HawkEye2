...
# -------------------------------------------------------------------------------
# Name:         hep_ipapicom
# Purpose:      HawkEye plug-in to identify the Geo-location of IP addresses
...
class hep_ipapicom(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "":
            self.error("You enabled hep_ipapicom but did not set an API key!")
            self.errorState = True
            return
        ...
        if data.get('country_name'):
            ...
            if data.get('latitude') and data.get('longitude'):
                evt = HawkEyeEvent("PHYSICAL_COORDINATES", f"{data.get('latitude')}, {data.get('longitude')}", self.__name__, event)
                self.notifyListeners(evt)
