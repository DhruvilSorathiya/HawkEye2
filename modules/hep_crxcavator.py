...
class hep_crxcavator(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        domain_keyword = self.he.domainKeyword(eventData, self.opts['_internettlds'])
        ...
        evt = HawkEyeEvent('RAW_RIR_DATA', json.dumps(results), self.__name__, event)
        ...
        for result in results:
            ...
            evt = HawkEyeEvent('RAW_RIR_DATA', json.dumps(extensions), self.__name__, event)
            self.notifyListeners(evt)
            ...
            for extension in extensions:
                ...
                if (
                    not self.getTarget().matches(self.he.urlFQDN(privacy_policy), includeChildren=True, includeParents=True)
                    and not self.getTarget().matches(self.he.urlFQDN(website), includeChildren=True, includeParents=True)
                    and not self.getTarget().matches(self.he.urlFQDN(offered_by), includeChildren=True, includeParents=True)
                    and not self.getTarget().matches(self.he.urlFQDN(support_site), includeChildren=True, includeParents=True)
                ):
                    self.debug(f"Extension {app_full_name} does not match {eventData}, skipping")
                    continue

                app_data = f"{name} {version}\n<HEURL>https://chrome.google.com/webstore/detail/{extension_id}</HEURL>"

                evt = HawkEyeEvent('APPSTORE_ENTRY', app_data, self.__name__, event)
                self.notifyListeners(evt)
        ...
        for url in set(urls):
            ...
            host = self.he.urlFQDN(url)

            if not host:
                continue

            if self.getTarget().matches(host, includeChildren=True, includeParents=True):
                evt = HawkEyeEvent('LINKED_URL_INTERNAL', url, self.__name__, event)
                self.notifyListeners(evt)
        ...
        for host in set(hosts):
            ...
            if self.opts['verify'] and not self.he.resolveHost(host) and not self.he.resolveHost6(host):
                self.debug(f"Host {host} could not be resolved")
                evt_type += '_UNRESOLVED'

            evt = HawkEyeEvent(evt_type, host, self.__name__, event)
            self.notifyListeners(evt)

        for location in set(locations):
            evt = HawkEyeEvent("PHYSICAL_ADDRESS", location, self.__name__, event)
            self.notifyListeners(evt)

# End of hep_crxcavator class
