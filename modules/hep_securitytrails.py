...
class hep_securitytrails(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "":
            self.error("You enabled hep_securitytrails but did not set an API uid/secret!")
            self.errorState = True
            return
                ...
                for r in rec:
                    if "host_provider" in r:
                        ...
                        for dat in r['host_provider']:
                            if dat in hosters:
                                continue
                            e = HawkEyeEvent("PROVIDER_HOSTING", dat,
                                                self.__name__, event)
                    ...
                    if "hostname" in r:
                        ...
                        e = HawkEyeEvent("CO_HOSTED_SITE", h, self.__name__, event)
                        self.notifyListeners(e)
                        self.cohostcount += 1
                ...
                for r in rec:
                    if "hostname" in r:
                        ...
                        if h.lower() not in myres:
                            ...
                            continue
                        e = HawkEyeEvent("AFFILIATE_INTERNET_NAME", h, self.__name__, event)
                        self.notifyListeners(e)

                        if self.he.isDomain(h, self.opts['_internettlds']):
                            evt = HawkEyeEvent("AFFILIATE_DOMAIN_NAME", h, self.__name__, event)
            ...
            if rec is not None:
                for h in rec:
                    ...
                    if h.lower() not in myres:
                        ...
                        continue
                    e = HawkEyeEvent("INTERNET_NAME", h + "." + domain,
                                        self.__name__, event)
                    self.notifyListeners(e)

# End of hep_securitytrails class
