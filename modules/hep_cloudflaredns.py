# -*- coding: utf-8 -*-
# -------------------------------------------------------------------------------
# Name:         hep_cloudflaredns
# Purpose:      HawkEye plug-in for looking up whether hosts are blocked by
...
class hep_cloudflaredns(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        family = self.he.normalizeDNS(self.queryFamilyDNS(eventData))
        malware = self.he.normalizeDNS(self.queryMalwareDNS(eventData))
        ...
        if '0.0.0.0' not in malware:
            ...
            evt = HawkEyeEvent(e, f"CloudFlare - Family [{eventData}]", self.__name__, event)
            self.notifyListeners(evt)
            return

        # Host is blocked only by malware filters
        self.debug(f"{eventData} blocked by CloudFlare Malware DNS")
        evt = HawkEyeEvent(e, f"CloudFlare - Malware [{eventData}]", self.__name__, event)
        self.notifyListeners(evt)
...
# End of hep_cloudflaredns class
