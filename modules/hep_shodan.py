...
class hep_shodan(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "":
            self.error("You enabled hep_shodan but did not set an API key!")
            self.errorState = True
            return
        ...
        if eventName == "DOMAIN_NAME":
            ...
            evt = HawkEyeEvent("RAW_RIR_DATA", str(hosts), self.__name__, event)
        ...
        if eventName == 'WEB_ANALYTICS_ID':
            ...
            evt = HawkEyeEvent("RAW_RIR_DATA", str(rec), self.__name__, event)
            self.notifyListeners(evt)
            return
        ...
        for addr in qrylist:
            ...
            if rec.get('os') is not None:
                evt = HawkEyeEvent("OPERATING_SYSTEM", f"{rec.get('os')} ({addr})", self.__name__, pevent)
                self.notifyListeners(evt)

            if rec.get('devtype') is not None:
                evt = HawkEyeEvent("DEVICE_TYPE", f"{rec.get('devtype')} ({addr})", self.__name__, pevent)
                self.notifyListeners(evt)

            if rec.get('country_name') is not None:
                location = ', '.join([_f for _f in [rec.get('city'), rec.get('country_name')] if _f])
                evt = HawkEyeEvent("GEOINFO", location, self.__name__, pevent)
                self.notifyListeners(evt)
            ...
            for r in rec['data']:
                ...
                if port is not None:
                    ...
                    if cp not in ports:
                        ports.append(cp)
                        evt = HawkEyeEvent("TCP_PORT_OPEN", cp, self.__name__, pevent)
                        self.notifyListeners(evt)

                if banner is not None:
                    if banner not in banners:
                        banners.append(banner)
                        evt = HawkEyeEvent("TCP_PORT_OPEN_BANNER", banner, self.__name__, pevent)
                        self.notifyListeners(evt)

                if product is not None:
                    if product not in products:
                        products.append(product)
                        evt = HawkEyeEvent("SOFTWARE_USED", product, self.__name__, pevent)
                        self.notifyListeners(evt)

                if asn is not None:
                    if asn not in asns:
                        asns.append(asn)
                        evt = HawkEyeEvent("BGP_AS_MEMBER", asn.replace("AS", ""), self.__name__, pevent)
                        self.notifyListeners(evt)
                ...
                if vulns is not None:
                    for vuln in vulns.keys():
                        if vuln not in vulnlist:
                            ...
                            etype, cvetext = self.he.cveInfo(vuln)
                            evt = HawkEyeEvent(etype, cvetext, self.__name__, pevent)
                            self.notifyListeners(evt)

# End of hep_shodan class
