...
# -------------------------------------------------------------------------------
# Name:        hep_dnsdb
# Purpose:     HawkEye plug-in that resolves and gets history of domains and IPs
#
...
class hep_dnsdb(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if self.opts["api_key"] == "":
            self.error("You enabled hep_dnsdb but did not set an API key!")
            self.errorState = True
            return
        ...
        if eventName == "DOMAIN_NAME":
            ...
            evt = HawkEyeEvent("RAW_RIR_DATA", str(rrsetRecords), self.__name__, event)
            ...
            for record in rrsetRecords:
                ...
                for data in record.get("rdata"):
                    ...
                    if record.get("rrtype") == "A":
                        if not self.he.validIP(data):
                            self.debug(f"Skipping invalid IP address {data}")
                            continue

                        if self.opts["verify"] and not self.he.validateIP(
                            eventData, data
                        ):
                            self.debug(
                        ...
                        evt = HawkEyeEvent("IP_ADDRESS", data, self.__name__, event)
                    ...
                    if record.get("rrtype") == "AAAA":
                        ...
                        evt = HawkEyeEvent("IPV6_ADDRESS", data, self.__name__, event)
                    elif record.get("rrtype") == "MX":
                        data = re.sub(r'.*\s+(.*)', r'\1', data)
                        evt = HawkEyeEvent("PROVIDER_MAIL", data, self.__name__, event)
                    elif record.get("rrtype") == "NS":
                        evt = HawkEyeEvent("PROVIDER_DNS", data, self.__name__, event)
                    elif record.get("rrtype") == "TXT":
                        data = data.replace('"', '')
                        evt = HawkEyeEvent("DNS_TEXT", data, self.__name__, event)
            ...
            evt = HawkEyeEvent("RAW_RIR_DATA", str(rdataRecords), self.__name__, event)
            self.notifyListeners(evt)
            ...
            for record in rdataRecords:
                ...
                if self.opts["verify"] and not self.he.resolveHost(data) and not self.he.resolveHost6(data):
                    self.debug(f"Host {data} could not be resolved")
                    evt = HawkEyeEvent("INTERNET_NAME_UNRESOLVED", data, self.__name__, event)
                else:
                    evt = HawkEyeEvent("INTERNET_NAME", data, self.__name__, event)
                self.notifyListeners(evt)

        for co in coHosts:
            if eventName == "IP_ADDRESS" and (
                self.opts["verify"] and not self.he.validateIP(co, eventData)
            ):
