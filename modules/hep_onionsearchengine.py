...
class hep_onionsearchengine(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        while keepGoing and page <= int(self.opts['max_pages']):
            ...
            data = self.he.fetchUrl('https://onionsearchengine.com/search.php?' + urllib.parse.urlencode(params),
            ...
            for link in links:
                ...
                if not self.he.urlFQDN(link).endswith(".onion"):
                    continue

                if not self.opts['fetchlinks']:
                    evt = HawkEyeEvent("DARKNET_MENTION_URL", link, self.__name__, event)
                    self.notifyListeners(evt)
                    continue

                res = self.he.fetchUrl(link,
...
# End of hep_onionsearchengine class
