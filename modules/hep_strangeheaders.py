# -*- coding: utf-8 -*-
# -------------------------------------------------------------------------------
# Name:         hep_strangeheaders
# Purpose:      HawkEye plug-in for identifying non-standard HTTP headers
...
class hep_strangeheaders(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        for key in data:
            if key.lower() not in headers:
                evt = HawkEyeEvent("WEBSERVER_STRANGEHEADER", f"{key}: {data[key]}", self.__name__, event)
                self.notifyListeners(evt)

# End of hep_strangeheaders class
