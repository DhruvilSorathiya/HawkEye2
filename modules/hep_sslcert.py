...
from hawkeye import HawkEyeEvent, HawkEyePlugin, HawkEyeHelpers


class hep_sslcert(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if eventName == "LINKED_URL_INTERNAL":
            ...
            try:
                ...
                if u.port:
                    port = u.port
                fqdn = self.he.urlFQDN(eventData.lower())
            except Exception:
        ...
        try:
            sock = self.he.safeSSLSocket(fqdn, port, self.opts['ssltimeout'])
            sock.do_handshake()
            dercert = sock.getpeercert(True)
            pemcert = HawkEyeHelpers.sslDerToPem(dercert)
            cert = self.he.parseCert(str(pemcert), fqdn, self.opts['certexpiringdays'])
        except Exception as x:
            self.info("Unable to SSL-connect to " + fqdn + " (" + str(x) + ")")
            return

        if eventName in ['INTERNET_NAME', 'IP_ADDRESS']:
            evt = HawkEyeEvent('TCP_PORT_OPEN', fqdn + ':' + str(port), self.__name__, event)
        ...
        rawevt = HawkEyeEvent("SSL_CERTIFICATE_RAW", cert['text'], self.__name__, event)
        self.notifyListeners(rawevt)

        if cert.get('issued'):
            evt = HawkEyeEvent('SSL_CERTIFICATE_ISSUED', cert['issued'], self.__name__, event)
            self.notifyListeners(evt)

        if cert.get('issuer'):
            evt = HawkEyeEvent('SSL_CERTIFICATE_ISSUER', cert['issuer'], self.__name__, event)
            self.notifyListeners(evt)

        if eventName != "IP_ADDRESS" and cert.get('mismatch'):
            evt = HawkEyeEvent('SSL_CERTIFICATE_MISMATCH', ', '.join(cert.get('hosts')), self.__name__, event)
            self.notifyListeners(evt)
        ...
        for san in set(cert.get('altnames', list())):
            ...
            if self.getTarget().matches(domain, includeChildren=True):
                ...
                if self.opts['verify'] and not self.he.resolveHost(domain) and not self.he.resolveHost6(domain):
                    self.debug(f"Host {domain} could not be resolved")
                    evt_type += '_UNRESOLVED'
            else:
                evt_type = 'CO_HOSTED_SITE'

            evt = HawkEyeEvent(evt_type, domain, self.__name__, event)
            self.notifyListeners(evt)

            if self.he.isDomain(domain, self.opts['_internettlds']):
                if evt_type == 'CO_HOSTED_SITE':
                    evt = HawkEyeEvent('CO_HOSTED_SITE_DOMAIN', domain, self.__name__, event)
                    self.notifyListeners(evt)
                else:
                    evt = HawkEyeEvent('DOMAIN_NAME', domain, self.__name__, event)
                    self.notifyListeners(evt)

        if cert.get('expired'):
            evt = HawkEyeEvent("SSL_CERTIFICATE_EXPIRED", cert.get('expirystr', 'Unknown'), self.__name__, event)
            self.notifyListeners(evt)
            return

        if cert.get('expiring'):
            evt = HawkEyeEvent("SSL_CERTIFICATE_EXPIRING", cert.get('expirystr', 'Unknown'), self.__name__, event)
            self.notifyListeners(evt)

# End of hep_sslcert class
