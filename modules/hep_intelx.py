...
class hep_intelx(HawkEyePlugin):
    ...
    def query(self, qry, qtype):
        ...
        res = self.he.fetchUrl(url, postData=json.dumps(payload),
        ...
        if ret.get('status', -1) == 0:
            ...
            while status in [3, 0] and count < limit:
                ...
                res = self.he.fetchUrl(resulturl, headers=headers)
                if res['content'] is None:
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "" or self.opts['base_url'] == "":
            self.error("You enabled hep_intelx but did not set an API key and/or base URL!")
            self.errorState = True
            return
        ...
        for info in data:
            for rec in info.get("records", dict()):
                ...
                e = HawkEyeEvent(evt, val, self.__name__, event)
        ...
        for info in data:
            for rec in info.get("selectors", dict()):
                ...
                if evt == "INTERNET_NAME" and self.he.isDomain(val, self.opts['_internettlds']):
                    e = HawkEyeEvent("DOMAIN_NAME", val, self.__name__, event)
                    self.notifyListeners(e)

# End of hep_intelx class
