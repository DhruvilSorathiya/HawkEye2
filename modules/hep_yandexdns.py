...
class hep_yandexdns(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        for result in res:
            ...
            evt = HawkEyeEvent(blacklist_type, f"{self.checks[k]} [{eventData}]", self.__name__, event)
            self.notifyListeners(evt)

            if k == '213.180.193.250':
                evt = HawkEyeEvent(malicious_type, f"{self.checks[k]} [{eventData}]", self.__name__, event)
                self.notifyListeners(evt)

# End of hep_yandexdns class
