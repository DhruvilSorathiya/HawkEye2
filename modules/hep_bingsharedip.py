...
class hep_bingsharedip(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "" and self.opts['api_key'] == "":
            self.error("You enabled hep_bingsharedip but did not set a Bing API key!")
        ...
        if eventName == "IP_ADDRESS" and srcModuleName == "hep_bingsharedip":
        ...
        for ip in qrylist:
            ...
            for url in urls:
                ...
                site = self.he.urlFQDN(url.lower())
                if site not in myres and site != ip:
                    if not self.opts["cohostsamedomain"]:
                        if self.getTarget().matches(site, includeParents=True):
                            ...
                            continue
                    if self.opts["verify"] and not self.he.validateIP(site, ip):
