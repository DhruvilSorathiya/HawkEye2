# -------------------------------------------------------------------------------
# Name:        hep_callername
# Purpose:     HawkEye plug-in to search CallerName.com for a phone number
...
class hep_callername(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        res = self.he.fetchUrl(url, timeout=self.opts['_fetchtimeout'], useragent=self.opts['_useragent'])
        ...
        if rep_good_match and rep_bad_match:
            ...
            if bad_votes > good_votes:
                ...
                evt = HawkEyeEvent('MALICIOUS_PHONE_NUMBER', text, self.__name__, event)
                self.notifyListeners(evt)

# End of hep_callername class
