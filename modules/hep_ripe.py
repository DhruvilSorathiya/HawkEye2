...
class hep_ripe(HawkEyePlugin):
    ...
    def findName(self, string):
        ...
        if self.keywords is None:
            self.keywords = self.he.domainKeywords(
    ...
    def handleEvent(self, event):
        ...
        if eventName == "BGP_AS_OWNER":
            ...
            for netblock in netblocks:
                ...
                if ":" in netblock:
                    evt = HawkEyeEvent("NETBLOCKV6_OWNER", netblock, self.__name__, event)
                    self.notifyListeners(evt)
                else:
                    evt = HawkEyeEvent("NETBLOCK_OWNER", netblock, self.__name__, event)
                    self.notifyListeners(evt)

            evt = HawkEyeEvent("RAW_RIR_DATA", self.lastContent, self.__name__, event)
            self.notifyListeners(evt)
        ...
        if eventName.startswith("NETBLOCK"):
            ...
            if eventName in ["NETBLOCK_OWNER", "NETBLOCKV6_OWNER"] and self.ownsAs(asn):
                asevt = HawkEyeEvent("BGP_AS_OWNER", asn, self.__name__, event)
                self.notifyListeners(asevt)
                evt = HawkEyeEvent("RAW_RIR_DATA", self.lastContent, self.__name__, event)
                self.notifyListeners(evt)
            else:
                asevt = HawkEyeEvent("BGP_AS_MEMBER", asn, self.__name__, event)
                self.notifyListeners(asevt)
        ...
        if eventName in ["IP_ADDRESS", "IPV6_ADDRESS"]:
            ...
            if not self.he.validIpNetwork(prefix):
                return
            ...
            if ":" in prefix:
                evt = HawkEyeEvent("NETBLOCKV6_" + relationship, prefix, self.__name__, event)
            else:
                evt = HawkEyeEvent("NETBLOCK_" + relationship, prefix, self.__name__, event)
            self.notifyListeners(evt)

# End of hep_ripe class
