...
# -------------------------------------------------------------------------------
# Name:         hep_tool_nmap
# Purpose:      HawkEye plug-in for using nmap to perform OS fingerprinting.
#
...
class hep_tool_nmap(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if srcModuleName == "hep_tool_nmap":
        ...
        if not self.opts['nmappath']:
            self.error("You enabled hep_tool_nmap but did not set a path to the tool!")
        ...
        if not self.he.validIP(eventData) and not self.he.validIpNetwork(eventData):
        ...
        if eventName == "IP_ADDRESS":
            try:
                ...
                if opsys:
                    evt = HawkEyeEvent("OPERATING_SYSTEM", opsys, self.__name__, event)
            ...
            try:
                ...
                for line in content.split('\n'):
                    ...
                    if opsys and currentIp:
                        ipevent = HawkEyeEvent("IP_ADDRESS", currentIp, self.__name__, event)
                        self.notifyListeners(ipevent)

                        evt = HawkEyeEvent("OPERATING_SYSTEM", opsys, self.__name__, ipevent)
                        self.notifyListeners(evt)
                        currentIp = None
            except Exception as e:
