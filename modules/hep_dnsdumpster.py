# -*- coding: utf-8 -*-
# -------------------------------------------------------------------------------
# Name:        hep_dnsdumpster
# Purpose:     HawkEye plug-in for subdomain enumeration using
#              dnsdumpster.com
#
...
class hep_dnsdumpster(HawkEyePlugin):
    ...
    def setup(self, hec, userOpts=dict()):
        self.he = hec
        self.debug("Setting up hep_dnsdumpster")
    ...
    def query(self, domain):
        ...
        res1 = self.he.fetchUrl(
            url,
            useragent=self.opts.get("_useragent", "Hawkeye")
        )
    ...
    def sendEvent(self, source, host):
        if self.he.resolveHost(host) or self.he.resolveHost6(host):
            e = HawkEyeEvent("INTERNET_NAME", host, self.__name__, source)
        else:
            e = HawkEyeEvent("INTERNET_NAME_UNRESOLVED", host, self.__name__, source)
    ...
    def handleEvent(self, event):
        ...
        eventDataHash = self.he.hashstring(query)
