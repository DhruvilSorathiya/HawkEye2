...
class hep_zoneh(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        content = self.he.cacheGet("hezoneh", 48)
        if content is None:
            ...
            self.he.cachePut("hezoneh", data['content'])
            content = data['content']

        ret = self.lookupItem(eventData, content)
        if ret:
            evt = HawkEyeEvent(evtType, ret, self.__name__, event)
            self.notifyListeners(evt)

# End of hep_zoneh class
