...
class hep_duckduckgo(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if self.opts['affiliatedomains'] and "AFFILIATE_" in eventName:
            eventData = self.he.hostDomain(eventData, self.opts['_internettlds'])
            if not eventData:
                return
        ...
        if abstract_text:
            ...
            evt = HawkEyeEvent(event_type, str(abstract_text), self.__name__, event)
        ...
        if related_topics:
            ...
            for topic in related_topics:
                ...
                evt = HawkEyeEvent(event_type, category, self.__name__, event)
                self.notifyListeners(evt)

# End of hep_duckduckgo class
