...
# -------------------------------------------------------------------------------
# Name:         hep_etherscan
# Purpose:      HawkEye plug-in to look up a ethereum wallet's balance by
...
class hep_etherscan(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "":
            self.error("You enabled hep_etherscan but did not set an API key!")
        ...
        evt = HawkEyeEvent("ETHEREUM_BALANCE", f"{str(balance)} ETH", self.__name__, event)
        self.notifyListeners(evt)

        evt = HawkEyeEvent("RAW_RIR_DATA", str(data), self.__name__, event)
        self.notifyListeners(evt)

# End of hep_etherscan class
