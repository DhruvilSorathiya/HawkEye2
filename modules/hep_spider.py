...
class hep_spider(HawkEyePlugin):

    meta = {
        'name': "Web hawk",
        'summary': "hawking of web-pages to extract content for searching.",
        'flags': ["slow"],
    ...
    optdescs = {
        ...
        'start': "Prepend targets with these until you get a hit, to start hawking.",
        ...
        'filterusers': "Skip hawking of /~user directories?",
        'nosubs': "Skip hawking of subdomains of the target?",
    ...
    def processUrl(self, url: str) -> dict:
        ...
        links = HawkEyeHelpers.extractLinksFromHtml(
            url,
            data,
        ...
        for link in links:
            if not self.opts['reportduplicates']:
                if link in self.urlEvents:
                    continue
            # Supply the HawkEyeEvent of the parent URL as the parent
    ...
    def cleanLinks(self, links: list) -> list:
        ...
        Returns:
            list: links suitable for hawking
        """
        returnLinks = dict()

        for link in links:
            linkBase = HawkEyeHelpers.urlBaseUrl(link)
            linkFQDN = self.he.urlFQDN(link)
            ...
            # All tests passed, add link to be hawked
            self.debug(f"Adding URL for hawking: {link}")
    ...
    def linkNotify(self, url: str, parentEvent=None):
        if self.getTarget().matches(self.he.urlFQDN(url)):
        ...
        event = HawkEyeEvent(utype, url, self.__name__, parentEvent)
    ...
    def handleEvent(self, event) -> None:
        ...
        # Don't hawk links we find ourselves
        if srcModuleName == "hep_spider":
            self.debug(f"Ignoring {eventName}, from self.")
            return None

        if eventData in self.urlEvents:
            self.debug(f"Ignoring {eventData} as already hawked or is being hawked.")
            return None

        self.urlEvents[eventData] = event

        # Determine where to start hawking from if it's a INTERNET_NAME event
        if eventName == "INTERNET_NAME":
            ...
            hawkTarget = eventData

        if not hawkTarget:
            self.info(f"No reply from {eventData}, aborting.")
            return None

        self.debug(f"Initiating hawk of {hawkTarget} from {srcModuleName}")

        # Link the hawked URL to the event that triggered it
        self.urlEvents[hawkTarget] = event
        return self.hawkFrom(hawkTarget)

    def hawkFrom(self, startingPoint: str) -> None:
        pagesFetched = 0
        levelsTraversed = 0

        # Are we respecting robots.txt?
        if self.opts['robotsonly']:
            targetBase = HawkEyeHelpers.urlBaseUrl(startingPoint)
            if targetBase not in self.robotsRules:
                ...
                if res:
                    ...
                    if robots_txt:
                        ...
                        self.robotsRules[targetBase] = HawkEyeHelpers.extractUrlsFromRobotsTxt(robots_txt)
        ...
        # number of hawked pages < max pages
        # hawking depth <= max levels (the first level is the first link)
        while (pagesFetched < self.opts['maxpages']) and (levelsTraversed <= self.opts['maxlevels']):
            if not nextLinks:
                self.info("No more links to hawk, finishing.")
                return
