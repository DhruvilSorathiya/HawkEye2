...
# -------------------------------------------------------------------------------
# Name:         hep_dnsbrute
# Purpose:      HawkEye plug-in for attempting to resolve through brute-forcing
#               common hostnames.
#
...
class hep_dnsbrute(HawkEyePlugin):
    ...
    def setup(self, hec, userOpts=dict()):
        ...
        if self.opts['commons']:
            with importlib.resources.open_text('hawkeye.dicts', 'subdomains.txt') as f:
                for s in f.readlines():
                    s = s.strip()
                    self.sublist[s] = True

        if self.opts['top10000']:
            with importlib.resources.open_text('hawkeye.dicts', 'subdomains-10000.txt') as f:
                for s in f.readlines():
                    s = s.strip()
                    self.sublist[s] = True
    ...
    def tryHost(self, name):
        try:
            if self.he.resolveHost(name) or self.he.resolveHost6(name):
                with self.lock:
    ...
    def tryHostWrapper(self, hostList, sourceEvent):
        ...
        for name in hostList:
            tn = 'thread_hep_dnsbrute_' + str(random.SystemRandom().randint(1, 999999999))
        ...
        while running:
            ...
            for rt in threading.enumerate():
                if rt.name.startswith("thread_hep_dnsbrute_"):
                    found = True
    ...
    def sendEvent(self, source, result):
        ...
        # Report the host
        evt = HawkEyeEvent("INTERNET_NAME", result, self.__name__, source)
    ...
    def handleEvent(self, event):
        ...
        if eventName == "INTERNET_NAME" and not self.getTarget().matches(eventData, includeChildren=False):
            ...
            wildcard = self.he.checkDnsWildcard(dom)
        ...
        wildcard = self.he.checkDnsWildcard(eventData)
        if self.opts['skipcommonwildcard'] and wildcard:
