# -*- coding: utf-8 -*-
# -------------------------------------------------------------------------------
# Name:         hep_dnszonexfer
# Purpose:      HawkEye plug-in for attempting a DNS zone transfer.
#
...
class hep_dnszonexfer(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if srcModuleName == "hep_dnszonexfer":
        ...
        # the zone transfer.
        if not self.he.validIP(eventData) and not self.he.validIP6(eventData):
            nsips = self.he.resolveHost(eventData)
            if not nsips:
                return
                ...
                for row in ret:
                    ...
                    if len(grps) > 0:
                        for strdata in grps:
                            ...
                            evt = HawkEyeEvent("INTERNET_NAME", strdata, self.__name__, parentEvent)
                            self.notifyListeners(evt)

            except Exception as e:
                self.info(f"Unable to perform DNS zone transfer for {eventData} ({name}): {e}")

# End of hep_dnszonexfer class
