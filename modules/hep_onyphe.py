...
# -------------------------------------------------------------------------------
# Name:         hep_onyphe
# Purpose:      HawkEye plug-in to check if the IP is included on Onyphe
...
class hep_onyphe(HawkEyePlugin):
    ...
    def emitLocationEvent(self, location, eventData, event):
        ...
        evt = HawkEyeEvent("PHYSICAL_COORDINATES", location, self.__name__, event)
    ...
    def emitDomainData(self, response, eventData, event):
        ...
        for domain in domains:
            if self.getTarget().matches(domain):
                if self.opts['verify']:
                    if self.he.resolveHost(domain) or self.he.resolveHost6(domain):
            ...
            if self.cohostcount < self.opts['maxcohost']:
                if self.opts["verify"] and not self.he.validateIP(domain, eventData):
                ...
                evt = HawkEyeEvent("CO_HOSTED_SITE", domain, self.__name__, event)
                self.notifyListeners(evt)
                self.cohostcount += 1
    ...
    def handleEvent(self, event):
        ...
        if self.opts["api_key"] == "":
            self.error("You enabled hep_onyphe, but did not set an API key!")
            self.errorState = True
            return
            ...
            for vulnerabilityData in vulnerabilityDataArr:
                ...
                for result in vulnerabilityData["results"]:
                    ...
                    for cve in cves:
                        ...
                        etype, cvetext = self.he.cveInfo(cve)
                        evt = HawkEyeEvent(
                            etype,
                            cvetext,
                            self.__name__,
