...
class hep_grep_app(HawkEyePlugin):
    ...
    def query(self, qry, page):
        ...
        res = self.he.fetchUrl("https://grep.app/api/search?" + urllib.parse.urlencode(params),
    ...
    def handleEvent(self, event):
        ...
        while page <= pages:
            ...
            for result in data:
                ...
                evt = HawkEyeEvent("RAW_RIR_DATA", str(result), self.__name__, event)
                ...
                links = self.he.extractUrlsFromText(snippet.replace('<mark>', '').replace('</mark>', ''))
                if links:
                    for link in links:
                        if link in self.results:
                            continue

                        host = self.he.urlFQDN(link)
                        ...
                        if not self.getTarget().matches(self.he.urlFQDN(link), includeChildren=True, includeParents=True):
                            self.debug("Skipped unrelated link: " + link)
                            continue

                        self.debug('Found a link: ' + link)
                        evt = HawkEyeEvent('LINKED_URL_INTERNAL', link, self.__name__, event)
                        self.notifyListeners(evt)
                        self.results[link] = True

                emails = HawkEyeHelpers.extractEmailsFromText(snippet.replace('<mark>', '').replace('</mark>', ''))
                if emails:
                    for email in emails:
        ...
        for host in set(hosts):
            ...
            if self.opts['dns_resolve'] and not self.he.resolveHost(host) and not self.he.resolveHost6(host):
                self.debug(f"Host {host} could not be resolved")
                evt = HawkEyeEvent("INTERNET_NAME_UNRESOLVED", host, self.__name__, event)
                self.notifyListeners(evt)
                continue

            evt = HawkEyeEvent("INTERNET_NAME", host, self.__name__, event)
            self.notifyListeners(evt)
            if self.he.isDomain(host, self.opts["_internettlds"]):
                evt = HawkEyeEvent("DOMAIN_NAME", host, self.__name__, event)
                self.notifyListeners(evt)

# End of hep_grep_app class
