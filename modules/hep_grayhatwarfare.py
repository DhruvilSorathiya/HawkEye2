...
class hep_grayhatwarfare(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "":
            self.error("You enabled hep_grayhatwarfare but did not set an API key!")
            self.errorState = True
            return
        ...
        while currentPage < maxPages:
            ...
            for row in data.get('buckets'):
                ...
                if bucketKeyword.startswith(keyword) or bucketKeyword.endswith(keyword):
                    evt = HawkEyeEvent('CLOUD_STORAGE_BUCKET', bucketName, self.__name__, event)
                    self.notifyListeners(evt)

                    evt = HawkEyeEvent('CLOUD_STORAGE_BUCKET_OPEN', f"{bucketName}: {row.get('fileCount')} files found.", self.__name__, event)
                    self.notifyListeners(evt)
...
# End of hep_grayhatwarfare class
