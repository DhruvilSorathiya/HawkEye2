...
class hep_venmo(HawkEyePlugin):
    ...
    # Query Venmo API
    def query(self, qry):
        res = self.he.fetchUrl('https://api.venmo.com/v1/users/' + qry,
    ...
    def handleEvent(self, event):
        ...
        if display_name:
            evt = HawkEyeEvent('HUMAN_NAME', display_name, self.__name__, event)
            self.notifyListeners(evt)

            evt = HawkEyeEvent('RAW_RIR_DATA', str(data),
                                  self.__name__, event)
            self.notifyListeners(evt)

# End of hep_venmo class
