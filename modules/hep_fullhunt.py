...
class hep_fullhunt(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        e = HawkEyeEvent("RAW_RIR_DATA", str(res), self.__name__, event)
        ...
        for record in res:
            ...
            if network_ports:
                for port in network_ports:
                    e = HawkEyeEvent("TCP_PORT_OPEN", f"{host}:{port}", self.__name__, event)
                    self.notifyListeners(e)
        ...
        for host in set(mail_servers):
            ...
            e = HawkEyeEvent("PROVIDER_MAIL", host, self.__name__, event)
            self.notifyListeners(e)
        ...
        for host in set(name_servers):
            ...
            e = HawkEyeEvent("PROVIDER_DNS", host, self.__name__, event)
            self.notifyListeners(e)
