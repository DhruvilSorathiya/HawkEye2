...
# -------------------------------------------------------------------------------
# Name:        hep_networksdb
# Purpose:     HawkEye plug-in to search NetworksDB.io API for IP address and
#              domain information.
#
...
class hep_networksdb(HawkEyePlugin):
    ...
    def queryIpInfo(self, qry):
        ...
        res = self.he.fetchUrl('https://networksdb.io/api/ip-info',
                               headers=headers,
    ...
    def queryIpGeo(self, qry):
        ...
        res = self.he.fetchUrl('https://networksdb.io/api/ip-geo',
                               headers=headers,
                               postData=urllib.parse.urlencode(params),
                               timeout=15,
                               useragent=self.opts['_useragent'])
    ...
    def queryReverseDns(self, qry):
        ...
        res = self.he.fetchUrl('https://networksdb.io/api/reverse-dns',
                               headers=headers,
                               postData=urllib.parse.urlencode(params),
                               timeout=15,
                               useragent=self.opts['_useragent'])
    ...
    def queryForwardDns(self, qry):
        ...
        res = self.he.fetchUrl('https://networksdb.io/api/dns',
                               headers=headers,
                               postData=urllib.parse.urlencode(params),
                               timeout=15,
                               useragent=self.opts['_useragent'])
    ...
    def queryAsnInfo(self, qry):
        ...
        res = self.he.fetchUrl('https://networksdb.io/api/asn',
                               headers=headers,
                               postData=urllib.parse.urlencode(params),
                               timeout=15,
                               useragent=self.opts['_useragent'])
    ...
    def queryAsnNetworks(self, qry):
        ...
        res = self.he.fetchUrl('https://networksdb.io/api/asn-networks',
                               headers=headers,
                               postData=urllib.parse.urlencode(params),
                               timeout=15,
                               useragent=self.opts['_useragent'])
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == '':
            self.error("You enabled hep_networksdb but did not set an API key!")
            self.errorState = True
            return
        ...
        if eventName in ["IP_ADDRESS", "IPV6_ADDRESS"]:
            ...
            if data is None:
                ...
                evt = HawkEyeEvent('RAW_RIR_DATA', str(data), self.__name__, event)
                self.notifyListeners(evt)

                network = data.get('network')
                if network:
                    cidr = network.get('cidr')
                    if cidr and cidr != 'N/A' and self.he.validIpNetwork(cidr):
                        if ":" in cidr:
                            evt = HawkEyeEvent('NETBLOCKV6_MEMBER', cidr, self.__name__, event)
                        else:
                            evt = HawkEyeEvent('NETBLOCK_MEMBER', cidr, self.__name__, event)
                        self.notifyListeners(evt)
            ...
            for co in set(cohosts):
                ...
                if self.opts['verify'] and not self.he.validateIP(co, eventData):
                    self.debug("Host " + co + " no longer resolves to " + eventData)
                    continue

                if not self.opts['cohostsamedomain']:
                    if self.getTarget().matches(co, includeParents=True):
                        evt = HawkEyeEvent('INTERNET_NAME', co, self.__name__, event)
                        self.notifyListeners(evt)
                        if self.he.isDomain(co, self.opts['_internettlds']):
                            evt = HawkEyeEvent('DOMAIN_NAME', co, self.__name__, event)
                            self.notifyListeners(evt)
                        continue

                if self.cohostcount < self.opts['maxcohost']:
                    evt = HawkEyeEvent('CO_HOSTED_SITE', co, self.__name__, event)
                    self.notifyListeners(evt)
                    self.cohostcount += 1
        ...
        if eventName in ["INTERNET_NAME", "DOMAIN_NAME"]:
            ...
            evt = HawkEyeEvent('RAW_RIR_DATA', str(res), self.__name__, event)
            self.notifyListeners(evt)

            for ip in res:
                if self.he.validIP(ip):
                    evt = HawkEyeEvent('IP_ADDRESS', ip, self.__name__, event)
                    self.notifyListeners(evt)
                elif self.he.validIP6(ip):
                    evt = HawkEyeEvent('IPV6_ADDRESS', ip, self.__name__, event)
                    self.notifyListeners(evt)

# End of hep_networksdb class
