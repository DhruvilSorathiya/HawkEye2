...
class hep_stevenblack_hosts(HawkEyePlugin):
    ...
    def retrieveBlocklist(self):
        blocklist = self.he.cacheGet('stevenblack_hosts', 24)
        ...
        self.he.cachePut("stevenblack_hosts", res['content'])
    ...
    def parseBlocklist(self, blocklist):
        ...
        for line in blocklist.split('\n'):
            ...
            # Note: Validation with he.validHost() is too slow to use here
            # if not self.he.validHost(host, self.opts['_internettlds']):
            #    continue
            if not host:
                continue
    ...
    def handleEvent(self, event):
        ...
        evt = HawkEyeEvent(malicious_type, text, self.__name__, event)
        self.notifyListeners(evt)

        evt = HawkEyeEvent(blacklist_type, text, self.__name__, event)
        self.notifyListeners(evt)

# End of hep_stevenblack_hosts class
