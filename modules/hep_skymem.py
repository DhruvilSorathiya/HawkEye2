...
# -------------------------------------------------------------------------------
# Name:         hep_skymem
# Purpose:      HawkEye plug-in for retrieving up to 100 e-mail addresses
...
class hep_skymem(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        res = self.he.fetchUrl("http://www.skymem.info/srch?q=" + eventData, timeout=self.opts['_fetchtimeout'], useragent=self.opts['_useragent'])

        if res['content'] is None:
            return

        # Extract emails from results page
        emails = HawkEyeHelpers.extractEmailsFromText(res['content'])
