...
class hep_emailrep(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == '' and not self.errorWarned:
            self.error("Warning: You enabled hep_emailrep but did not set an API key! Queries will be rate limited.")
        ...
        if credentials_leaked:
            evt = HawkEyeEvent('EMAILADDR_COMPROMISED', eventData + " [Unknown]", self.__name__, event)
            self.notifyListeners(evt)

        malicious_activity = details.get('malicious_activity')
        if malicious_activity:
            evt = HawkEyeEvent('MALICIOUS_EMAILADDR', 'EmailRep [' + eventData + ']', self.__name__, event)
            self.notifyListeners(evt)
