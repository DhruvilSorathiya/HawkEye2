...
class hep_haveibeenpwned(HawkEyePlugin):
    ...
    def query(self, qry):
        ...
        while retry < 2:
            ...
            res = self.he.fetchUrl(url, timeout=self.opts['_fetchtimeout'],
                                   useragent="HawkEye", headers=hdrs)
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "":
            self.error("You enabled hep_haveibeenpwned but did not set an API key!")
            self.errorState = True
            return
        ...
        if data is not None:
            for n in data:
                ...
                if eventName == 'EMAILADDR':
                    e = HawkEyeEvent("EMAILADDR_COMPROMISED", eventData + " [" + site + "]",
                                        self.__name__, event)
                else:
                    e = HawkEyeEvent("PHONE_NUMBER_COMPROMISED", eventData + " [" + site + "]",
                                        self.__name__, event)
        ...
        for link in links:
            try:
                ...
                res = self.he.fetchUrl(link, timeout=self.opts['_fetchtimeout'], useragent=self.opts['_useragent'])
                ...
                evt1 = HawkEyeEvent("LEAKSITE_URL", link, self.__name__, event)
                self.notifyListeners(evt1)

                evt2 = HawkEyeEvent("LEAKSITE_CONTENT", res['content'], self.__name__, evt1)
...
# End of hep_haveibeenpwned class
