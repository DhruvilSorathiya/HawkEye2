...
class hep_robtex(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        for ip in qrylist:
            ...
            while retry < retries:
                ...
                res = self.he.fetchUrl("https://freeapi.robtex.com/ipquery/" + ip, timeout=self.opts['_fetchtimeout'])
            ...
            evt = HawkEyeEvent("RAW_RIR_DATA", json.dumps(data), self.__name__, event)
            ...
            for r in data.get('pas'):
                ...
                if self.opts['verify'] and not self.he.validateIP(host, ip):
                ...
                if eventName == "NETBLOCK_OWNER":
                    ...
                    evt = HawkEyeEvent("CO_HOSTED_SITE", host, self.__name__, ipe)
                    self.notifyListeners(evt)
                elif eventName == "NETBLOCKV6_OWNER":
                    ipe = HawkEyeEvent("IPV6_ADDRESS", ip, self.__name__, event)
                    self.notifyListeners(ipe)
                    evt = HawkEyeEvent("CO_HOSTED_SITE", host, self.__name__, ipe)
                    self.notifyListeners(evt)
                else:
                    evt = HawkEyeEvent("CO_HOSTED_SITE", host, self.__name__, event)
                    self.notifyListeners(evt)

                self.cohostcount += 1

# End of hep_robtex class
