...
class hep_virustotal(HawkEyePlugin):
    ...
    optdescs = {
        ...
        'publicapi': 'Are you using a public key? If so HawkEye will pause for 15 seconds after each query to avoid VirusTotal dropping requests.',
    ...
    def handleEvent(self, event):
        ...
        for addr in qrylist:
            ...
            if len(info.get('detected_urls', [])) > 0:
                ...
                infourl = f"<HEURL>https://www.virustotal.com/en/{infotype}/{addr}/information/</HEURL>"
            ...
            for domain in set(domains):
                ...
                if self.he.isDomain(domain, self.opts['_internettlds']):
                    if evt_type.startswith('AFFILIATE'):
                        evt = HawkEyeEvent('AFFILIATE_DOMAIN_NAME', domain, self.__name__, event)
                        self.notifyListeners(evt)
                    else:
                        evt = HawkEyeEvent('DOMAIN_NAME', domain, self.__name__, event)
                        self.notifyListeners(evt)

# End of hep_virustotal class
