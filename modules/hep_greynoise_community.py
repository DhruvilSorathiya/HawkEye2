...
class hep_greynoise_community(HawkEyePlugin):
    ...
    def queryIP(self, qry, qry_type):
        ...
        if qry_type == "ip":
            ...
            ip_response = self.he.fetchUrl(
                ...
                useragent="greynoise-hawkeye-community-v1.2.0",
                headers=headers,
            )
    ...
    def handleEvent(self, event):
        ...
        if self.opts["api_key"] == "":
            self.error("You enabled hep_greynoise_community but did not set an API key!")
            self.errorState = True
            return
        ...
        if "noise" in ret:
            if ret.get("noise", None):
                ...
                if ret.get("name", "unknown") != "unknown":
                    e = HawkEyeEvent("COMPANY_NAME", ret.get("name"), self.__name__, event)
                    self.notifyListeners(e)
...
# End of hep_greynoise_community class
