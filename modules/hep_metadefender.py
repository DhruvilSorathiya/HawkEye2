...
class hep_metadefender(HawkEyePlugin):
    ...
    def queryDomain(self, qry):
        ...
        res = self.he.fetchUrl('https://api.metadefender.com/v4/domain/' + qry,
                               headers=headers,
                               timeout=15,
    ...
    def queryIp(self, qry):
        ...
        res = self.he.fetchUrl('https://api.metadefender.com/v4/ip/' + qry,
                               headers=headers,
                               timeout=15,
                               useragent=self.opts['_useragent'])
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "":
            self.error("You enabled hep_metadefender but did not set an API key!")
            self.errorState = True
            return
        ...
        if eventName == 'IP_ADDRESS':
            ...
            for m in sources:
                ...
                evt = HawkEyeEvent('MALICIOUS_IPADDR', provider + ' [' + eventData + ']', self.__name__, event)
                self.notifyListeners(evt)
                evt = HawkEyeEvent('BLACKLISTED_IPADDR', provider + ' [' + eventData + ']', self.__name__, event)
                self.notifyListeners(evt)
        ...
        if eventName == 'INTERNET_NAME':
            ...
            for m in sources:
                ...
                if m['assessment'] == "trustworthy":
                    continue
                provider = m.get('provider')
                evt = HawkEyeEvent('MALICIOUS_INTERNET_NAME', provider + ' [' + eventData + ']', self.__name__, event)
                self.notifyListeners(evt)
                evt = HawkEyeEvent('BLACKLISTED_INTERNET_NAME', provider + ' [' + eventData + ']', self.__name__, event)
                self.notifyListeners(evt)

# End of hep_metadefender class
