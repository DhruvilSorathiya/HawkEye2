...
# -------------------------------------------------------------------------------
# Name:         hep_crt
# Purpose:      HawkEye plug-in to identify historical certificates for a domain
...
class hep_crt(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        for cert_id in set(certs):
            ...
            try:
                cert = self.he.parseCert(str(res['content']))
            except Exception as e:
            ...
            if cert_text:
                evt = HawkEyeEvent("SSL_CERTIFICATE_RAW", str(cert_text), self.__name__, event)
                self.notifyListeners(evt)
        ...
        for domain in set(domains):
            ...
            if not self.he.validHost(domain, self.opts['_internettlds']):
                continue
