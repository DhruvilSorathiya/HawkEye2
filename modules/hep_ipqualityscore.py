# -*- coding: utf-8 -*-
# -------------------------------------------------------------------------------
# Name:         hep_ipqualityscore
# Purpose:      Hawkeye module to check whether a target is malicious
...
class hep_ipqualityscore(HawkEyePlugin):
    ...
    def setup(self, hec, userOpts=None):
        if userOpts is None:
            userOpts = {}
        self.he = hec
    ...
    def handleEvent(self, event):
        ...
        if eventName == "PHONE_NUMBER":
            ...
            if phoneNumberType:
                evt = HawkEyeEvent("PHONE_NUMBER_TYPE", phoneNumberType, self.__name__, event)
                self.notifyListeners(evt)
            ...
            if data.get('leaked'):
                evt = HawkEyeEvent("EMAILADDR_COMPROMISED", f"{eventData} [Unknown]", self.__name__, event)
                self.notifyListeners(evt)
            ...
            if malicious:
                ...
                if eventName == "IP_ADDRESS":
                    ...
                    evt = HawkEyeEvent("MALICIOUS_INTERNET_NAME", maliciousDesc, self.__name__, event)
                self.notifyListeners(evt)
...
# End of hep_ipqualityscore class
