...
class hep_riskiq(HawkEyePlugin):
    ...
    def query(self, qry, qtype, opts=dict()):
        ...
        res = self.he.fetchUrl(url, timeout=30,
                               useragent="HawkEye", headers=headers,
                               postData=post)
    ...
    def handleEvent(self, event):
            ...
            if not ret:
                ...
                for res in ret:
                    ...
                    if self.getTarget().matches(host, includeChildren=True):
                        if self.he.resolveHost(host) or self.he.resolveHost6(host):
                            e = HawkEyeEvent("INTERNET_NAME", host, self.__name__, event)
                        else:
                            e = HawkEyeEvent("INTERNET_NAME_UNRESOLVED", host, self.__name__, event)
                        self.notifyListeners(e)

                        if self.he.isDomain(host, self.opts['_internettlds']):
                            e = HawkEyeEvent("DOMAIN_NAME", host, self.__name__, event)
                            self.notifyListeners(e)
        ...
        if eventName == 'EMAILADDR':
            ...
            for r in ret:
                if not eventData.endswith("@" + r['domain']):
                    if self.he.validIP(r['domain']):
                        t = "NETBLOCK_OWNER"
                    else:
                        t = "AFFILIATE_INTERNET_NAME"
                    e = HawkEyeEvent(t, r['domain'], self.__name__, event)
                    self.notifyListeners(e)

                    if t == "AFFILIATE_INTERNET_NAME" and self.he.isDomain(r['domain'], self.opts['_internettlds']):
                        evt = HawkEyeEvent("AFFILIATE_DOMAIN_NAME", r['domain'], self.__name__, event)
        ...
        if eventName in ['IP_ADDRESS', 'INTERNET_NAME', 'DOMAIN_NAME']:
            ...
            for co in cohosts:
                ...
                if eventName == "IP_ADDRESS" and (self.opts['verify'] and not self.he.validateIP(co, eventData)):
                    self.debug("Host no longer resolves to our IP.")
                    continue

                if not self.opts['cohostsamedomain']:
                    if self.getTarget().matches(co, includeParents=True):
                        if self.he.resolveHost(co) or self.he.resolveHost6(co):
                            e = HawkEyeEvent("INTERNET_NAME", co, self.__name__, event)
                        else:
                            e = HawkEyeEvent("INTERNET_NAME_UNRESOLVED", co, self.__name__, event)
                        self.notifyListeners(e)

                        if self.he.isDomain(co, self.opts['_internettlds']):
                            e = HawkEyeEvent("DOMAIN_NAME", co, self.__name__, event)
                            self.notifyListeners(e)
                        continue

                if self.cohostcount < self.opts['maxcohost']:
                    e = HawkEyeEvent("CO_HOSTED_SITE", co, self.__name__, event)
                    self.notifyListeners(e)
                    self.cohostcount += 1

# End of hep_riskiq class
