...
class hep_fraudguard(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if self.opts['fraudguard_api_key_account'] == "" or self.opts['fraudguard_api_key_password'] == "":
            self.error("You enabled hep_fraudguard but did not set an API username/password!")
            self.errorState = True
            return
        ...
        for addr in qrylist:
            ...
            if geoinfo:
                ...
                e = HawkEyeEvent("GEOINFO", location, self.__name__, pevent)
                self.notifyListeners(e)

            threat = data.get('threat')
            if threat and threat != "unknown":
                risk_level = data.get('risk_level')
                e = HawkEyeEvent(evtType, f"{threat} (risk level: {risk_level}) [{addr}]", self.__name__, pevent)
                self.notifyListeners(e)

# End of hep_fraudguard class
