...
class hep_zonefiles(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if ip:
            if self.opts['verify']:
                if self.he.validateIP(eventData, str(ip)):
                    evt = HawkEyeEvent('IP_ADDRESS', str(ip), self.__name__, event)
                    self.notifyListeners(evt)
            else:
                evt = HawkEyeEvent('IP_ADDRESS', str(ip), self.__name__, event)
                self.notifyListeners(evt)
        ...
        if dns:
            for nameserver in set(dns.split(',')):
                evt = HawkEyeEvent('PROVIDER_DNS', nameserver, self.__name__, event)
                self.notifyListeners(evt)
        ...
        if phones:
            for phone in set(phones.split(',')):
                if HawkEyeHelpers.validPhoneNumber(phone):
        ...
        if technologies and isinstance(technologies, dict):
            for tech in technologies.keys():
                evt = HawkEyeEvent('SOFTWARE_USED', tech, self.__name__, event)
                self.notifyListeners(evt)

# End of hep_zonefiles class
