...
class hep_viewdns(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "":
            self.error("You enabled hep_viewdns but did not set an API key!")
            self.errorState = True
            return
        ...
        for domain in set(myres):
            ...
            if eventName == "EMAILADDR":
                e = HawkEyeEvent("AFFILIATE_INTERNET_NAME", domain, self.__name__, event)
                ...
                if eventName in ["IP_ADDRESS", "IPV6_ADDRESS"] and self.opts['verify'] and not self.he.validateIP(domain, eventData):
                    self.debug(f"Host {domain} no longer resolves to IP address: {eventData}")
                    continue

                self.cohostcount += 1

                e = HawkEyeEvent("CO_HOSTED_SITE", domain, self.__name__, event)
                self.notifyListeners(e)

# End of hep_viewdns class
