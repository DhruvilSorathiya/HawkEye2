...
class hep_pageinfo(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if "hep_spider" not in event.module:
        ...
        for regexpGrp in regexps:
            ...
            for regex in regexps[regexpGrp]:
                ...
                if len(matches) > 0 and regexpGrp not in self.results[eventSource]:
                    ...
                    evt = HawkEyeEvent(regexpGrp, eventSource, self.__name__, event)
        ...
        if len(self.results[eventSource]) == 0:
            ...
            evt = HawkEyeEvent("URL_STATIC", eventSource, self.__name__, event)
            self.notifyListeners(evt)
        ...
        if len(matches) > 0:
            for match in matches:
                if '://' not in match:
                    continue
                if not self.he.urlFQDN(match):
                    continue
                if self.getTarget().matches(self.he.urlFQDN(match)):
                    continue
                self.debug(f"Externally hosted JavaScript found at: {match}")
                evt = HawkEyeEvent("PROVIDER_JAVASCRIPT", match, self.__name__, event)
                self.notifyListeners(evt)

# End of hep_pageinfo class
