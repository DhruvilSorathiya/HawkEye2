...
class hep_socialprofiles(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if self.opts['google_api_key'] == "" and self.opts['bing_api_key'] == "":
            self.error("You enabled hep_socialprofiles but did not set a Google or Bing API key!")
        ...
        for site in sites:
            ...
            for searchDom in sites[site][1]:
                ...
                for match in matches:
                    ...
                    match = urllib.parse.unquote(match)
                    evt = HawkEyeEvent(
                        "SOCIAL_MEDIA", site + ": <HEURL>" + match + "</HEURL>", self.__name__, event
                    )
                    self.notifyListeners(evt)


# End of hep_socialprofiles class
