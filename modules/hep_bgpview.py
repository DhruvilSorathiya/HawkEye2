...
class hep_bgpview(HawkEyePlugin):
    ...
    def queryAsn(self, qry):
        res = self.he.fetchUrl("https://api.bgpview.io/asn/" + qry.replace('AS', ''),
    ...
    def queryIp(self, qry):
        res = self.he.fetchUrl("https://api.bgpview.io/ip/" + qry,
                               useragent=self.opts['_useragent'],
                               timeout=self.opts['_fetchtimeout'])
    ...
    def queryNetblock(self, qry):
        res = self.he.fetchUrl("https://api.bgpview.io/prefix/" + qry,
                               useragent=self.opts['_useragent'],
                               timeout=self.opts['_fetchtimeout'])
    ...
    def handleEvent(self, event):
        ...
        if eventName == 'BGP_AS_MEMBER':
            ...
            evt = HawkEyeEvent('PHYSICAL_ADDRESS', ', '.join([_f for _f in address if _f]), self.__name__, event)
        ...
        if eventName in ['IP_ADDRESS', 'IPV6_ADDRESS']:
            ...
            for prefix in prefixes:
                ...
                evt = HawkEyeEvent("BGP_AS_MEMBER", str(asn), self.__name__, event)
                self.notifyListeners(evt)

                if self.he.validIpNetwork(p):
                    if ":" in p:
                        evt = HawkEyeEvent("NETBLOCKV6_MEMBER", p, self.__name__, event)
                    else:
                        evt = HawkEyeEvent("NETBLOCK_MEMBER", p, self.__name__, event)
                    self.notifyListeners(evt)

# End of hep_bgpview class
