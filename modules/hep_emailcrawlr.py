...
class hep_emailcrawlr(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "":
            self.error("You enabled hep_emailcrawlr but did not set an API key!")
            self.errorState = True
            return
            ...
            for res in emails:
                ...
                if name:
                    ...
                    if full_name and len(full_name) > 3:
                        evt = HawkEyeEvent("RAW_RIR_DATA", f"Possible full name: {full_name}",
                                              self.__name__, event)
                        self.notifyListeners(evt)
                ...
                if phone_numbers:
                    for number in phone_numbers:
                        if number:
                            evt = HawkEyeEvent("PHONE_NUMBER", number, self.__name__, event)
                            self.notifyListeners(evt)
                ...
                if location:
                    ...
                    if loc:
                        evt = HawkEyeEvent("GEOINFO", loc, self.__name__, event)
                        self.notifyListeners(evt)

# End of hep_emailcrawlr class
