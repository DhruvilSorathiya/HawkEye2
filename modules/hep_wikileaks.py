...
class hep_wikileaks(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        links = dict()
        p = HawkEyeHelpers.extractLinksFromHtml(wlurl, res['content'], "wikileaks.org")
        if p:
            links.update(p)

        p = HawkEyeHelpers.extractLinksFromHtml(wlurl, res['content'], "cryptome.org")
        if p:
            links.update(p)
            ...
            for link in links:
                ...
                if link.count('/') >= 4:
                    if not link.endswith(".js") and not link.endswith(".css"):
                        evt = HawkEyeEvent("LEAKSITE_URL", link, self.__name__, event)
