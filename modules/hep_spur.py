...
# -------------------------------------------------------------------------------
# Name:         hep_spur
# Purpose:      Hawkeye plugin to search spur.us API for any
...
class hep_spur(HawkEyePlugin):
    ...
    # For a list of all events, check hedb.py.
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "":
            self.error("You enabled hep_spur but did not set an API key!")
            self.errorState = True
            return
        ...
        for addr in qrylist:
            ...
            if eventName.startswith("NETBLOCK_"):
                ipEvt = HawkEyeEvent("IP_ADDRESS", addr, self.__name__, event)
                self.notifyListeners(ipEvt)
                evt = HawkEyeEvent("RAW_RIR_DATA", str(data), self.__name__, ipEvt)
            ...
            if geoTag:
                ...
                if eventName.startswith("NETBLOCK_"):
                    evt = HawkEyeEvent("GEOINFO", geoInfo, self.__name__, ipEvt)
                    self.notifyListeners(evt)
            ...
            if asData:
                ...
                if orgName:
                    if eventName.startswith("NETBLOCK_"):
                        evt = HawkEyeEvent("COMPANY_NAME", orgName, self.__name__, ipEvt)
                        self.notifyListeners(evt)
                    elif eventName.startswith("AFFILIATE_"):
                        # Don't report COMPANY_NAME for Affiliates
                        pass
                    else:
                        evt = HawkEyeEvent("COMPANY_NAME", orgName, self.__name__, event)
                        self.notifyListeners(evt)
            ...
            if vpnOperatorsExists:
                ...
                if eventName.startswith("NETBLOCK_"):
                    evt = HawkEyeEvent("MALICIOUS_IPADDR", maliciousIPDesc, self.__name__, ipEvt)
                    self.notifyListeners(evt)
                elif eventName.startswith("AFFILIATE_"):
                    evt = HawkEyeEvent("MALICIOUS_AFFILIATE_IPADDR", maliciousIPDesc, self.__name__, event)
                    self.notifyListeners(evt)
                else:
                    evt = HawkEyeEvent("MALICIOUS_IPADDR", maliciousIPDesc, self.__name__, event)
                    self.notifyListeners(evt)

# End of hep_spur class
