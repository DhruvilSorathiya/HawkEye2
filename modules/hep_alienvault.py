...
class hep_alienvault(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if eventName == 'INTERNET_NAME':
            ...
            for url in set(urls):
                ...
                host = self.he.urlFQDN(url.lower())
                ...
                if passive_dns:
                    ...
                    for rec in passive_dns:
                        ...
                        if self.getTarget().matches(host, includeParents=True):
                            ...
                            evt = HawkEyeEvent(evtType, host, self.__name__, event)
                            self.notifyListeners(evt)
                            continue
                        ...
                        if self.opts["verify"] and not self.he.validateIP(host, eventData):
        ...
        for addr in qrylist:
            ...
            if rec.get("reputation", None):
                ...
                if eventName == 'NETBLOCKV6_OWNER':
                    ...
                    pevent = HawkEyeEvent("AFFILIATE_IPV6_ADDRESS", addr, self.__name__, event)
                    self.notifyListeners(pevent)
                else:
                    pevent = event

                e = HawkEyeEvent(evtType, descr, self.__name__, pevent)
                self.notifyListeners(e)

# End of hep_alienvault class
