...
class hep_binaryedge(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if eventName == "EMAILADDR":
            ...
            for rec in ret:
                ...
                for leak in events:
                    e = HawkEyeEvent('EMAILADDR_COMPROMISED', f"{eventData} [{leak}]", self.__name__, event)
            ...
            for rec in ret:
                ...
                for rec in events:
                    ...
                    if self.cohostcount < self.opts['maxcohost']:
                        e = HawkEyeEvent(evtType, host, self.__name__, event)
                        self.notifyListeners(e)
                        self.cohostcount += 1
        ...
        if eventName == "DOMAIN_NAME":
            ...
            for rec in ret:
                ...
                for rec in events:
                    ...
                    if self.opts['verify'] and not self.he.resolveHost(rec) and not self.he.resolveHost6(rec):
                        self.debug(f"Couldn't resolve {rec}, so skipping.")
                        continue

                    e = HawkEyeEvent('INTERNET_NAME', rec, self.__name__, event)
                    self.notifyListeners(e)
        ...
        for addr in qrylist:
            ...
            for rec in ret:
                ...
                for rec in events:
                    ...
                    e = HawkEyeEvent('MALICIOUS_IPADDR', dat, self.__name__, event)
                    self.notifyListeners(e)
        ...
        for addr in qrylist:
            ...
            for rec in ret:
                ...
                for rec in results:
                    ...
                    if cves:
                        for c in cves:
                            etype, cvetext = self.he.cveInfo(c['cve'])
                            e = HawkEyeEvent(etype, cvetext, self.__name__, event)
                            self.notifyListeners(e)
                ...
                for res in events:
                    for prec in res['results']:
                        ...
                        if f"{evttype}:{port}" not in ports:
                            ev = HawkEyeEvent(evttype, entity, self.__name__, event)
                        ...
                        e = HawkEyeEvent(evtbtype, banner, self.__name__, ev)
                        self.notifyListeners(e)

        for addr in qrylist:
            self.checkedips[addr] = True

# End of hep_binaryedge class
