...
class hep_social(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        for regexpGrp in list(regexps.keys()):
            for regex in regexps[regexpGrp]:
                ...
                evt = HawkEyeEvent(
                    "SOCIAL_MEDIA", f"{regexpGrp}: <HEURL>{eventData}</HEURL>",
                    self.__name__,
                    event
                )
                ...
                if regexpGrp != "Google+":
                    un = bits.group(1)
                    evt = HawkEyeEvent("USERNAME", str(un), self.__name__, event)
                    self.notifyListeners(evt)

# End of hep_social class
