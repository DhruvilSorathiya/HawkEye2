# -*- coding: utf-8 -*-
# -------------------------------------------------------------------------------
# Name:        hep_tool_nbtscan
# Purpose:     HawkEye plug-in for using the nbtscan tool.
...
class hep_tool_nbtscan(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if srcModuleName == "hep_tool_nbtscan":
            self.debug("Skipping event from myself.")
            return

        if not self.opts['nbtscan_path']:
            self.error("You enabled hep_tool_nbtscan but did not set a path to the tool!")
        ...
        for row in stdout.split("\n"):
            ...
            if not inside and len(info) > 0:
                ...
                if eventName == "NETBLOCK_OWNER":
                    ...
                    srcEvent = HawkEyeEvent("IP_ADDRESS", addr, self.__name__, event)
                    self.notifyListeners(srcEvent)

                evt = HawkEyeEvent('UDP_PORT_OPEN', f"{addr}:137", self.__name__, srcEvent)
                self.notifyListeners(evt)

                evt = HawkEyeEvent('UDP_PORT_OPEN_INFO', info, self.__name__, evt)
                self.notifyListeners(evt)
                info = ""

# End of hep_tool_nbtscan class
