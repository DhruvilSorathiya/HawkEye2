...
class hep_hostio(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if ipinfo and isinstance(ipinfo, dict):
            for address, ip_data in data["ipinfo"].items():
                ...
                if not self.he.validIP(address):
                    continue
                evt = HawkEyeEvent("IP_ADDRESS", address, self.__name__, event)
                self.notifyListeners(evt)
                found = True

                loc = ip_data.get("loc")
                if loc and isinstance(loc, str):
                    loc_evt = HawkEyeEvent(
