...
# -------------------------------------------------------------------------------
# Name:         hep_dnsresolve
# Purpose:      HawkEye plug-in for extracting hostnames from identified data
#               and resolving them.
#
...
class hep_dnsresolve(HawkEyePlugin):
    ...
    def enrichTarget(self, target):
        ...
        for host in ret:
            ...
            if self.he.validIP(host):
                target.setAlias(host, "IP_ADDRESS")
            elif self.he.validIP6(host):
                ...
                #     dom = self.he.hostDomain(host, self.opts['_internettlds'])
    ...
    def resolveTargets(self, target, validateReverse: bool) -> list:
        ...
        Args:
            target (HawkEyeTarget): target object
        ...
        if t in ["IP_ADDRESS", "IPV6_ADDRESS"]:
            r = self.he.resolveIP(v)
            if r:
                ret.extend(r)
        if t == "INTERNET_NAME":
            r = self.he.resolveHost(v)
            if r:
                ret.extend(r)
            r = self.he.resolveHost6(v)
            if r:
                ret.extend(r)
        if t == "NETBLOCK_OWNER":
            ...
            for addr in IPNetwork(v):
                ...
                names = self.he.resolveIP(ipaddr)
                if not names:
                    continue
    ...
    def handleEvent(self, event) -> None:
        ...
        eventDataHash = self.he.hashstring(eventData)
        addrs = None
        parentEvent = event

        # Don't be recursive for names
        if srcModuleName in ["hep_dnsresolve"] and "_NAME" in eventName:
            return
        ...
        if eventData.endswith(".ipv6-literal.net") and eventName == "AFFILIATE_INTERNET_NAME":
            ...
            if self.he.validIP6(ipv6):
                if self.getTarget().matches(ipv6):
                    evt = HawkEyeEvent("IPV6_ADDRESS", ipv6, self.__name__, parentEvent)
                else:
                    evt = HawkEyeEvent("AFFILIATE_IPV6_ADDRESS", ipv6, self.__name__, parentEvent)
        ...
        if eventData.endswith(".in-addr.arpa") and eventName == "AFFILIATE_INTERNET_NAME":
            ...
            if self.he.validIP(ipv4):
                if self.getTarget().matches(ipv4):
                    evt = HawkEyeEvent("IP_ADDRESS", ipv4, self.__name__, parentEvent)
                else:
                    evt = HawkEyeEvent("AFFILIATE_IPADDR", ipv4, self.__name__, parentEvent)
                self.notifyListeners(evt)
        ...
        if eventName in ["CO_HOSTED_SITE", "AFFILIATE_INTERNET_NAME"]:
            ...
            if self.he.isDomain(eventData, self.opts['_internettlds']):
                evt = HawkEyeEvent(ev, eventData, self.__name__, parentEvent)
                self.notifyListeners(evt)

            # In case the domain of the provided host is different, report that too
            dom = self.he.hostDomain(eventData, self.opts['_internettlds'])
            if dom and dom != eventData:
                evt = HawkEyeEvent(ev, dom, self.__name__, parentEvent)
                self.notifyListeners(evt)

        # Resolve host names
        if eventName in ["INTERNET_NAME", "AFFILIATE_INTERNET_NAME"]:
            addrs = list()
            addrs.extend(self.he.resolveHost(eventData))
            addrs.extend(self.he.resolveHost6(eventData))
        ...
        # Reverse resolve IP addresses
            ...
            addrs = self.he.resolveIP(eventData)
            ...
            for ip in IPNetwork(eventData):
                ...
                if self.he.validIP(ipaddr):
                    if ipaddr.split(".")[3] in ['255', '0']:
                        continue
                    if '255' in ipaddr.split("."):
                        continue

                addrs = self.he.resolveIP(ipaddr)
                if not addrs:
                    continue
    ...
    def processHost(self, host, parentEvent, affiliate=None) -> None:
        parentHash = self.he.hashstring(parentEvent.data)
        ...
        if affiliate is None:
            ...
            # list of aliases,
            if not self.he.validIP(host):
                hostips = self.he.resolveHost(host)
                if hostips:
                    for hostip in hostips:
                        if self.getTarget().matches(hostip):
                            affil = False
                            break
                hostips6 = self.he.resolveHost6(host)
                if hostips6:
        ...
        if affil:
            if self.he.isValidLocalOrLoopbackIp(host):
                htype = "INTERNAL_IP_ADDRESS"
            elif self.he.validIP(host):
        ...
        if htype in ["INTERNET_NAME", "AFFILIATE_INTERNET_NAME"]:
            if not self.he.resolveHost(host) and not self.he.resolveHost6(host):
                evt = HawkEyeEvent(f"{htype}_UNRESOLVED", host, self.__name__, parentEvent)
                self.notifyListeners(evt)
                return

        # Report the host
        if host != parentEvent.data:
            evt = HawkEyeEvent(htype, host, self.__name__, parentEvent)
            self.notifyListeners(evt)
        else:
            evt = parentEvent
        ...
        if htype == "INTERNET_NAME":
            ...
            ip6s = self.he.resolveHost6(host)
            if not ip6s:
                return
            for ip6 in ip6s:
                parentHash = self.he.hashstring(evt.data)
                ...
                evt6 = HawkEyeEvent("IPV6_ADDRESS", ip6, self.__name__, evt)
        ...
        if htype == "AFFILIATE_INTERNET_NAME":
            ...
            if not dom:
                return
            if dom == host and not self.he.isDomain(dom, self.opts['_internettlds']):
                return
    ...
    def processDomain(self, domainName, parentEvent, affil=False, host=None) -> None:
        ...
        if affil:
            domevt = HawkEyeEvent("AFFILIATE_DOMAIN_NAME", domainName,
                                     self.__name__, parentEvent)
            self.notifyListeners(domevt)
            return

        if self.getTarget().matches(domainName):
            domevt = HawkEyeEvent("DOMAIN_NAME", domainName,
                                     self.__name__, parentEvent)
            self.notifyListeners(domevt)
        else:
            ...
            if parentEvent.data.endswith("." + domainName):
                domevt = HawkEyeEvent("DOMAIN_NAME_PARENT", domainName,
                                         self.__name__, parentEvent)
                self.notifyListeners(domevt)

# End of hep_dnsresolve class
