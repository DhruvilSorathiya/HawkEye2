...
class hep_github(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if eventName == "SOCIAL_MEDIA":
            ...
            e = HawkEyeEvent("RAW_RIR_DATA", f"Possible full name: {full_name}", self.__name__, event)
        ...
        if eventName == "DOMAIN_NAME":
            username = self.he.domainKeyword(eventData, self.opts['_internettlds'])
            if not username:
                return
        ...
        if not failed:
            for item in ret['items']:
                ...
                if repo_info is not None:
                    ...
                    evt = HawkEyeEvent("PUBLIC_CODE_REPO", repo_info, self.__name__, event)
            ...
            for item in ret['items']:
                ...
                for item in repret:
                    ...
                    if repo_info is not None:
                        ...
                        evt = HawkEyeEvent("PUBLIC_CODE_REPO", repo_info,
                                              self.__name__, event)
                        self.notifyListeners(evt)


# End of hep_github class
