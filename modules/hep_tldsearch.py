...
# -------------------------------------------------------------------------------
# Name:         hep_tldsearch
# Purpose:      HawkEye plug-in for identifying the existence of this target
#               on other TLDs.
#
...
class hep_tldsearch(HawkEyePlugin):
    ...
    def tryTld(self, target, tld):
        ...
        if self.opts['skipwildcards'] and self.he.checkDnsWildcard(tld):
            return

        try:
            if not self.he.resolveHost(target) and not self.he.resolveHost6(target):
                with self.lock:
    ...
    def tryTldWrapper(self, tldList, sourceEvent):
        ...
        for i, pair in enumerate(tldList):
            (domain, tld) = pair
            tn = 'thread_hep_tldsearch_' + str(random.SystemRandom().randint(0, 999999999))
        ...
        while running:
            ...
            for rt in threading.enumerate():
                if rt.name.startswith("thread_hep_tldsearch_"):
                    found = True
    ...
    def sendEvent(self, source, result):
        ...
        if self.opts['activeonly']:
            ...
            pageContent = self.he.fetchUrl('http://' + result,
                                           timeout=self.opts['_fetchtimeout'],
                                           useragent=self.opts['_useragent'],
                                           noLog=True,
                                           verify=False)
            if pageContent['content'] is not None:
                evt = HawkEyeEvent("SIMILARDOMAIN", result, self.__name__, source)
    ...
    def handleEvent(self, event):
        ...
        keyword = self.he.domainKeyword(eventData, self.opts['_internettlds'])
