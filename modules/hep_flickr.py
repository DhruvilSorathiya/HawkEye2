...
class hep_flickr(HawkEyePlugin):
    ...
    def retrieveApiKey(self):
        res = self.he.fetchUrl("https://www.flickr.com/", timeout=self.opts['_fetchtimeout'], useragent=self.opts['_useragent'])
    ...
    def query(self, qry, api_key, page=1, per_page=200):
        ...
        res = self.he.fetchUrl("https://api.flickr.com/services/rest?" + urllib.parse.urlencode(params),
    ...
    def handleEvent(self, event):
        ...
        while page <= pages:
            ...
            for photo in photos.get('photo', list()):
                emails = HawkEyeHelpers.extractEmailsFromText(str(photo))
                for email in emails:
                ...
                links = HawkEyeHelpers.extractUrlsFromText(str(photo))
                for link in links:
