...
class hep_webserver(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if not self.getTarget().matches(self.he.urlFQDN(eventSource)):
        ...
        if 'location' in jdata:
            if jdata['location'].startswith('http://') or jdata['location'].startswith('https://'):
                if self.getTarget().matches(self.he.urlFQDN(jdata['location'])):
                    evt = HawkEyeEvent('LINKED_URL_INTERNAL', jdata['location'], self.__name__, event)
                    self.notifyListeners(evt)
                else:
                    evt = HawkEyeEvent('LINKED_URL_EXTERNAL', jdata['location'], self.__name__, event)
                    self.notifyListeners(evt)
            ...
            for directive in jdata['content-security-policy'].split(';'):
                for string in directive.split(' '):
                    if string.startswith('http://') or string.startswith('https://'):
                        if self.getTarget().matches(self.he.urlFQDN(string)):
                            evt = HawkEyeEvent('LINKED_URL_INTERNAL', string, self.__name__, event)
                            self.notifyListeners(evt)
                        else:
                            evt = HawkEyeEvent('LINKED_URL_EXTERNAL', string, self.__name__, event)
                            self.notifyListeners(evt)
        ...
        for t in set(tech):
            evt = HawkEyeEvent("WEBSERVER_TECHNOLOGY", t, self.__name__, event)
            self.notifyListeners(evt)

# End of hep_webserver class
