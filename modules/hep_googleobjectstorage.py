# -*- coding: utf-8 -*-
# -------------------------------------------------------------------------------
# Name:         hep_googleobjectstorage
# Purpose:      HawkEye plug-in for identifying potential Google Object Storage
...
class hep_googleobjectstorage(HawkEyePlugin):
    ...
    def checkSite(self, url):
        res = self.he.fetchUrl(url, timeout=10, useragent="HawkEye", noLog=True)
    ...
    def threadSites(self, siteList):
        ...
        for site in siteList:
            ...
            t.append(threading.Thread(name='thread_hep_googleobjectstorage_' + tname,
        ...
        while running:
            ...
            for rt in threading.enumerate():
                if rt.name.startswith("thread_hep_googleobjectstorage_"):
                    found = True
    ...
    def handleEvent(self, event):
        ...
        if eventName == "LINKED_URL_EXTERNAL":
            if ".storage.googleapis.com" in eventData:
                b = self.he.urlFQDN(eventData)
                evt = HawkEyeEvent("CLOUD_STORAGE_BUCKET", b, self.__name__, event)
                self.notifyListeners(evt)
            return

        targets = [eventData.replace('.', '')]
        kw = self.he.domainKeyword(eventData, self.opts['_internettlds'])
        if kw:
            targets.append(kw)
        ...
        for b in ret:
            bucket = b.split(":")
            evt = HawkEyeEvent("CLOUD_STORAGE_BUCKET", bucket[0] + ":" + bucket[1], self.__name__, event)
            self.notifyListeners(evt)
            if bucket[2] != "0":
                bucketname = bucket[1].replace("//", "")
                evt = HawkEyeEvent("CLOUD_STORAGE_BUCKET_OPEN", bucketname + ": " + bucket[2] + " files found.",
                                      self.__name__, evt)
                self.notifyListeners(evt)


# End of hep_googleobjectstorage class
