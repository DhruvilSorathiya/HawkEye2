# -*- coding: utf-8 -*-
# -------------------------------------------------------------------------------
# Name:         hep_tool_nuclei
# Purpose:      HawkEye plug-in for using the 'Nuclei' tool.
...
class hep_tool_nuclei(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if srcModuleName == "hep_tool_nuclei":
            return

        if not self.opts['nuclei_path'] or not self.opts['template_path']:
            self.error("You enabled hep_tool_nuclei but did not set a path to the tool and/or templates!")
        ...
        try:
            for line in content.split("\n"):
                ...
                if host != eventData:
                    ...
                    srcevent = HawkEyeEvent(srctype, host, self.__name__, event)
                ...
                if matches:
                    ...
                    if "matcher-name" in data:
                        ...
                        if data['info'].get('reference'):
                            datatext += f"Reference: <HEURL>{data['info']['reference'][0]}</HEURL>"
