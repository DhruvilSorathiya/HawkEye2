...
class hep_stackoverflow(HawkEyePlugin):
    ...
    def extractIP4s(self, text):
        ...
        for m in matches:
            if self.he.validIP(m) and not self.he.isValidLocalOrLoopbackIP(m):
                ips.add(m)
    ...
    def extractIP6s(self, text):
        ...
        for m in matches:
            if self.he.validIP6(m) and not self.he.isValidLocalOrLoopbackIP(m):
                ips.add(m)
    ...
    def handleEvent(self, event):
        ...
        for item in items:
            ...
            e = HawkEyeEvent(
                'RAW_RIR_DATA',
                f"<HEURL>https://stackoverflow.com/questions/{question}</HEURL>\n{item}",
                self.__name__,
                event
            )
            self.notifyListeners(e)

            emails = HawkEyeHelpers.extractEmailsFromText(text)
            if emails:
                for email in emails:
        ...
        for email in set(allEmails):
            ...
            if self.getTarget().matches(email):
                e = HawkEyeEvent('EMAILADDR', email, self.__name__, event)
            else:
                e = HawkEyeEvent('AFFILIATE_EMAILADDR', email, self.__name__, event)
            self.notifyListeners(e)

        for username in set(allUsernames):
            if " " in username:
                e = HawkEyeEvent('RAW_RIR_DATA', 'Possible full name: ' + username, self.__name__, event)
            else:
                e = HawkEyeEvent('USERNAME', username, self.__name__, event)
            self.notifyListeners(e)

        for ip in set(allIP4s):
            ip = str(ip)
            e = HawkEyeEvent('AFFILIATE_IP_ADDRESS', ip, self.__name__, event)
            self.notifyListeners(e)

        for ip in set(allIP6s):
            ip = str(ip)
            e = HawkEyeEvent('AFFILIATE_IPV6_ADDRESS', ip, self.__name__, event)
            self.notifyListeners(e)

# End of hep_stackoverflow class
