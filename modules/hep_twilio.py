...
class hep_twilio(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key_account_sid'] == "" or self.opts['api_key_auth_token'] == "":
            self.error("You enabled hep_twilio but did not set account sid/auth token")
        ...
        if callerName:
            evt = HawkEyeEvent("COMPANY_NAME", callerName, self.__name__, event)
            self.notifyListeners(evt)

# End of hep_twilio class
