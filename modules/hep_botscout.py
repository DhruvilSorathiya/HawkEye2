...
class hep_botscout(HawkEyePlugin):
    ...
    def queryEmail(self, email):
        if not HawkEyeHelpers.validEmail(email):
            return None
    ...
    def handleEvent(self, event):
        ...
        if not self.opts['api_key']:
            self.info("You enabled hep_botscout but did not set an API key! Queries will be limited to 100 per day.")
        ...
        if eventName == "IP_ADDRESS":
            ...
            evt = HawkEyeEvent("MALICIOUS_IPADDR", text, self.__name__, event)
            self.notifyListeners(evt)

            evt = HawkEyeEvent("BLACKLISTED_IPADDR", text, self.__name__, event)
            self.notifyListeners(evt)
            ...
            evt = HawkEyeEvent("MALICIOUS_EMAILADDR", text, self.__name__, event)
            self.notifyListeners(evt)
        else:
            self.debug(f"Unexpected event type {eventName}, skipping")

# End of hep_botscout class
