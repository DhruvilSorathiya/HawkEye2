# -*- coding: utf-8 -*-
# -------------------------------------------------------------------------------
# Name:        hep_tool_wafw00f
# Purpose:     HawkEye plug-in for using the WAFW00F tool.
...
class hep_tool_wafw00f(HawkEyePlugin):
    meta = {
        'name': "Tool - WAFW00F",
    ...
    def handleEvent(self, event):
        ...
        if not self.opts['wafw00f_path']:
            self.error("You enabled hep_tool_wafw00f but did not set a path to the tool!")
        ...
        if not HawkEyeHelpers.sanitiseInput(url):
        ...
        evt = HawkEyeEvent('RAW_RIR_DATA', json.dumps(result_json), self.__name__, event)
