...
# -------------------------------------------------------------------------------
# Name:        hep_mnemonic
# Purpose:     HawkEye plug-in for retrieving passive DNS information
...
class hep_mnemonic(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        while position < (per_page * max_pages):
            ...
            for r in data:
                ...
                if eventName in ['INTERNET_NAME', 'DOMAIN_NAME']:
                    ...
                    if r['rrtype'] == 'a':
                        if not self.he.validIP(answer):
                            continue

                        if self.he.isValidLocalOrLoopbackIp(answer):
                            evt = HawkEyeEvent("INTERNAL_IP_ADDRESS", answer, self.__name__, event)
                        else:
                            evt = HawkEyeEvent("IP_ADDRESS", answer, self.__name__, event)
                        self.notifyListeners(evt)

                    if r['rrtype'] == 'aaaa':
                        if not self.he.validIP6(r['answer']):
                            continue

                        if self.he.isValidLocalOrLoopbackIp(answer):
                            evt = HawkEyeEvent("INTERNAL_IP_ADDRESS", answer, self.__name__, event)
                        else:
                            evt = HawkEyeEvent("IPV6_ADDRESS", answer, self.__name__, event)
                        self.notifyListeners(evt)
        ...
        for co in set(cohosts):
            ...
            if self.getTarget().matches(co, includeParents=True):
                if self.opts['verify'] and not self.he.resolveHost(co) and not self.he.resolveHost6(co):
                    self.debug(f"Host {co} could not be resolved")
                    evt = HawkEyeEvent("INTERNET_NAME_UNRESOLVED", co, self.__name__, event)
                    self.notifyListeners(evt)
                    continue
