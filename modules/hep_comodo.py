...
class hep_comodo(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if not self.he.resolveHost(eventData) and not self.he.resolveHost6(eventData):
            return
        ...
        evt = HawkEyeEvent(blacklist_type, f"Comodo Secure DNS [{eventData}]", self.__name__, event)
        self.notifyListeners(evt)

        evt = HawkEyeEvent(malicious_type, f"Comodo Secure DNS [{eventData}]", self.__name__, event)
        self.notifyListeners(evt)

# End of hep_comodo class
