...
# -------------------------------------------------------------------------------
# Name:         hep_dnsraw
# Purpose:      HawkEye plug-in for collecting raw DNS records.
...
class hep_dnsraw(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        for rec in list(recs.keys()):
            ...
            for x in res.answer:
                ...
                evt = HawkEyeEvent("RAW_DNS_RECORDS", str(x), self.__name__, parentEvent)
                ...
                for rx in list(recs.keys()):
                    ...
                    for m in grps:
                        ...
                        if rx == "MX":
                            evt = HawkEyeEvent("PROVIDER_MAIL", strdata.lower(), self.__name__, parentEvent)
                            self.notifyListeners(evt)
                            domains.append(strdata.lower())

                        if rx == "NS":
                            evt = HawkEyeEvent("PROVIDER_DNS", strdata.lower(), self.__name__, parentEvent)
                            self.notifyListeners(evt)
                            domains.append(strdata.lower())

                        if rx == "TXT":
                            evt = HawkEyeEvent("DNS_TEXT", strdata, self.__name__, parentEvent)
                            self.notifyListeners(evt)

                            if "v=spf" in strdata or "spf2.0/" in strdata:
                                evt = HawkEyeEvent("DNS_SPF", strdata, self.__name__, parentEvent)
                                self.notifyListeners(evt)
        ...
        for domain in set(domains):
            ...
            evt = HawkEyeEvent(evt_type, domain, self.__name__, parentEvent)
            self.notifyListeners(evt)

# End of hep_dnsraw class
