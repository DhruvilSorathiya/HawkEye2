...
class hep_onioncity(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "":
            self.error("You enabled hep_onioncity but did not set a Google API key!")
        ...
        darknet_links = [
            link for link in new_links if self.he.urlFQDN(link).endswith(".onion.link")
        ]
        ...
        for link in darknet_links:
            ...
            if self.opts['fetchlinks']:
                res = self.he.fetchUrl(torlink, timeout=self.opts['_fetchtimeout'],
                                       useragent=self.opts['_useragent'],
                                       verify=False)
                ...
                evt = HawkEyeEvent("DARKNET_MENTION_URL", torlink, self.__name__, event)
                self.notifyListeners(evt)
                ...
                evt = HawkEyeEvent("DARKNET_MENTION_CONTENT", "..." + data + "...",
                                      self.__name__, evt)
                self.notifyListeners(evt)
            else:
                evt = HawkEyeEvent("DARKNET_MENTION_URL", torlink, self.__name__, event)
