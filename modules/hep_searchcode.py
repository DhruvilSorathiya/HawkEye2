...
class hep_searchcode(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        while page < max_pages:
            ...
            emails = HawkEyeHelpers.extractEmailsFromText(str(results))
            for email in emails:
                ...
                evt = HawkEyeEvent(evt_type, email, self.__name__, event)
            ...
            for result in results:
                ...
                if lines:
                    for line in lines:
                        links.update(HawkEyeHelpers.extractUrlsFromText(lines[line]))
            ...
            for result in results:
                ...
                evt = HawkEyeEvent('PUBLIC_CODE_REPO', repo_data, self.__name__, event)
                self.notifyListeners(evt)

                evt = HawkEyeEvent('RAW_RIR_DATA', json.dumps(result), self.__name__, event)
                self.notifyListeners(evt)
