...
class hep_builtwith(HawkEyePlugin):
    ...
    def queryRelationships(self, t):
        ...
        res = self.he.fetchUrl(url, timeout=self.opts['_fetchtimeout'], useragent="HawkEye")
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "":
            self.error("You enabled hep_builtwith but did not set an API key!")
            ...
            if "Meta" in data:
                if data['Meta'].get("Names", []):
                    for nb in data['Meta']['Names']:
                        e = HawkEyeEvent("RAW_RIR_DATA", "Possible full name: " + nb['Name'],
                                            self.__name__, event)
                        self.notifyListeners(e)
                        if nb.get('Email', None):
                            if HawkEyeHelpers.validEmail(nb['Email']):
                                if nb['Email'].split("@")[0] in self.opts['_genericusers'].split(","):
                                    evttype = "EMAILADDR_GENERIC"
                                else:
                                    evttype = "EMAILADDR"
                                e = HawkEyeEvent(evttype, nb['Email'],
                                                    self.__name__, event)
                                self.notifyListeners(e)
                ...
                if data['Meta'].get("Telephones", []):
                    for phone in data['Meta']['Telephones']:
                        ...
                        e = HawkEyeEvent("PHONE_NUMBER", phone, self.__name__, event)
                        self.notifyListeners(e)
            ...
            if "Paths" in data.get("Result", []):
                for p in data["Result"]['Paths']:
                    if p.get("SubDomain", ""):
                        ...
                        ev = HawkEyeEvent("INTERNET_NAME", h, self.__name__, event)
                        self.notifyListeners(ev)
                        if self.he.isDomain(h, self.opts['_internettlds']):
                            ev = HawkEyeEvent("DOMAIN_NAME", h, self.__name__, event)
                            self.notifyListeners(ev)
                    else:
                        ev = None
                    ...
                    for t in p.get("Technologies", []):
                        ...
                        if t.get("LastDetected", 0) < agelimit:
                            ...
                            continue
                        e = HawkEyeEvent("WEBSERVER_TECHNOLOGY", t["Name"],
                                            self.__name__, src)
                        self.notifyListeners(e)
        ...
        for r in data:
            ...
            for i in r['Identifiers']:
                ...
                if i['Type'] == "ip":
                    ...
                    e = HawkEyeEvent(evttype, val, self.__name__, event)
                    self.notifyListeners(e)
                    continue

                # Related through shared analytics ID
                txt = i['Type'] + ": " + str(i['Value'])
                e = HawkEyeEvent("WEB_ANALYTICS_ID", txt, self.__name__, event)
                self.notifyListeners(e)
                ...
                if i['Matches']:
                    for m in i['Matches']:
                        if "Domain" not in m:
                            continue
                        evt = HawkEyeEvent("AFFILIATE_INTERNET_NAME", m['Domain'], self.__name__, e)
                        self.notifyListeners(evt)

                        if self.he.isDomain(m['Domain'], self.opts['_internettlds']):
                            evt = HawkEyeEvent("AFFILIATE_DOMAIN_NAME", m['Domain'], self.__name__, e)
                            self.notifyListeners(evt)

# End of hep_builtwith class
