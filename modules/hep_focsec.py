...
class hep_focsec(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if is_bot:
            e = HawkEyeEvent("MALICIOUS_IPADDR", f"Focsec [{eventData}]", self.__name__, event)
            self.notifyListeners(e)

        is_tor = data.get('is_tor')
        if is_tor:
            e = HawkEyeEvent("TOR_EXIT_NODE", eventData, self.__name__, event)
            self.notifyListeners(e)

        is_vpn = data.get('is_vpn')
        if is_vpn:
            e = HawkEyeEvent("VPN_HOST", eventData, self.__name__, event)
            self.notifyListeners(e)

        is_proxy = data.get('is_proxy')
        if is_proxy:
            e = HawkEyeEvent("PROXY_HOST", eventData, self.__name__, event)
            self.notifyListeners(e)
