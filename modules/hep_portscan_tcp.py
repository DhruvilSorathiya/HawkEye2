# -*- coding: utf-8 -*-
# -------------------------------------------------------------------------------
# Name:         hep_portscan_tcp
# Purpose:      HawkEye plug-in for performing a basic TCP port scan of IP
...
class hep_portscan_tcp(HawkEyePlugin):
    ...
    def setup(self, hec, userOpts=dict()):
        ...
        if self.opts['ports'][0].startswith("http://") or \
            ...
            file_ports = self.he.optValueToData(self.opts['ports'][0])
            if file_ports:
    ...
    def tryPort(self, ip, port):
        ...
        try:
            sock = self.he.safeSocket(ip, port, self.opts['timeout'])
            with self.lock:
    ...
    def tryPortWrapper(self, ip, portList):
        ...
        while i < len(portList):
            ...
            t.append(threading.Thread(name=f"hep_portscan_tcp_{port}", target=self.tryPort, args=(ip, port)))
            t[i].start()
            i += 1
        ...
        while running:
            ...
            for rt in threading.enumerate():
                if rt.name.startswith("hep_portscan_tcp_"):
                    found = True
    ...
    def sendEvent(self, resArray, srcEvent):
        for cp in resArray:
            ...
            evt = HawkEyeEvent("TCP_PORT_OPEN", cp, self.__name__, srcEvent)
            self.notifyListeners(evt)

            if resArray[cp] is not True:
                banner = str(resArray[cp], 'utf-8', errors='replace')
                bevt = HawkEyeEvent("TCP_PORT_OPEN_BANNER", banner, self.__name__, evt)
