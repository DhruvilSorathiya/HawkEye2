# -*- coding: utf-8 -*-
# -------------------------------------------------------------------------------
# Name:         hep_tool_retirejs
# Purpose:      HawkEye plug-in for using the 'Retire.js' tool.
#
...
class hep_tool_retirejs(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if not self.opts['retirejs_path']:
            self.error("You enabled hep_tool_retirejs but did not set a path to the tool!")
                ...
                for result in item["results"]:
                    for vuln in result["vulnerabilities"]:
                        if "CVE" not in vuln["identifiers"]:
                            ...
                            text += f"Info: <HEURL>{vuln['info'][0]}</HEURL>"
                            evt = HawkEyeEvent(
...
# End of hep_tool_retirejs class
