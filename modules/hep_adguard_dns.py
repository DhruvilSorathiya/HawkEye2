...
class hep_adguard_dns(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        default = self.he.normalizeDNS(self.queryDefaultDNS(eventData))
        ...
        if '94.140.14.35' in family:
            ...
            evt = HawkEyeEvent(blacklist_type, f"AdGuard - Family Filter [{eventData}]", self.__name__, event)
            self.notifyListeners(evt)

        if '94.140.14.35' in default:
            self.debug(f"{eventData} blocked by AdGuard Default DNS")
            evt = HawkEyeEvent(blacklist_type, f"AdGuard - Default Filter [{eventData}]", self.__name__, event)
            self.notifyListeners(evt)

# End of hep_adguard_dns class
