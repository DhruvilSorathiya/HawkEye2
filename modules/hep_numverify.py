# -------------------------------------------------------------------------------
# Name:        hep_numverify
# Purpose:     HawkEye plug-in to search numverify.com API for a phone number
...
class hep_numverify(HawkEyePlugin):
    ...
    def query(self, qry):
        ...
        res = self.he.fetchUrl("http://apilayer.net/api/validate?" + urllib.parse.urlencode(params),
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "":
            self.error("You enabled hep_numverify but did not set an API key!")
            self.errorState = True
            return
        ...
        if data.get('country_code'):
            country = HawkEyeHelpers.countryNameFromCountryCode(data.get('country_code'))
        ...
        if data.get('carrier'):
            evt = HawkEyeEvent("PROVIDER_TELCO", data.get('carrier'), self.__name__, event)
            self.notifyListeners(evt)
        else:
            self.debug("No carrier information found for " + eventData)

# End of hep_numverify class
