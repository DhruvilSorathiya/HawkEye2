# -*- coding: utf-8 -*-
# -------------------------------------------------------------------------------
# Name:        hep_tool_whatweb
# Purpose:     HawkEye plug-in for using the 'WhatWeb' tool.
...
class hep_tool_whatweb(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if not self.opts['whatweb_path']:
            self.error("You enabled hep_tool_whatweb but did not set a path to the tool!")
        ...
        if not HawkEyeHelpers.sanitiseInput(eventData):
        ...
        for result in result_json:
            ...
            if plugin_matches.get('HTTPServer'):
                for w in plugin_matches.get('HTTPServer').get('string'):
                    evt = HawkEyeEvent('WEBSERVER_BANNER', w, self.__name__, event)
                    self.notifyListeners(evt)
                    found = True

            if plugin_matches.get('X-Powered-By'):
                for w in plugin_matches.get('X-Powered-By').get('string'):
                    evt = HawkEyeEvent('WEBSERVER_TECHNOLOGY', w, self.__name__, event)
                    self.notifyListeners(evt)
                    found = True
            ...
            for plugin in plugin_matches:
                if plugin in blacklist:
                    continue
                evt = HawkEyeEvent('WEBSERVER_TECHNOLOGY', plugin, self.__name__, event)
                self.notifyListeners(evt)
                found = True

        if found:
            evt = HawkEyeEvent('RAW_RIR_DATA', str(result_json), self.__name__, event)
            self.notifyListeners(evt)

# End of hep_tool_whatweb class
