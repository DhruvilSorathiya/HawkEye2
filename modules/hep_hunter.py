...
class hep_hunter(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "":
            self.error("You enabled hep_hunter but did not set an API key!")
        ...
        while rescount <= maxgoal:
            for email in data['data'].get('emails', list()):
                ...
                e = HawkEyeEvent(evttype, em, self.__name__, event)
                ...
                if 'first_name' in email and 'last_name' in email:
                    if email['first_name'] is not None and email['last_name'] is not None:
                        ...
                        e = HawkEyeEvent("RAW_RIR_DATA", "Possible full name: " + n,
                                            self.__name__, event)
                        self.notifyListeners(e)
