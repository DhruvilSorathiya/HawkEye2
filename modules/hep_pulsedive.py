...
class hep_pulsedive(HawkEyePlugin):
    ...
    def query(self, qry):
        ...
        res = self.he.fetchUrl(url, timeout=30, useragent="HawkEye")
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "":
            self.error("You enabled hep_pulsedive but did not set an API key!")
            self.errorState = True
            return
        ...
        for addr in qrylist:
            ...
            if attributes:
                ...
                if ports:
                    for p in ports:
                        e = HawkEyeEvent('TCP_PORT_OPEN', addr + ':' + p, self.__name__, event)
            ...
            for result in threats:
                ...
                if tid:
                    descr += "\n<HEURL>https://pulsedive.com/indicator/?iid=" + tid + "</HEURL>"
