...
class hep_xforce(HawkEyePlugin):
    ...
    def query(self, qry, querytype):
        ...
        res = self.he.fetchUrl(url, timeout=self.opts['_fetchtimeout'], useragent="HawkEye", headers=headers)
    ...
    def handleEvent(self, event):
        ...
        if self.opts['xforce_api_key'] == "" or self.opts['xforce_api_key_password'] == "":
            self.error("You enabled hep_xforce but did not set an API key/password!")
            self.errorState = True
            return
        ...
        for addr in qrylist:
            ...
            if rec:
                ...
                if len(rec_history) > 0:
                    ...
                    e = HawkEyeEvent("RAW_RIR_DATA", str(rec_history), self.__name__, event)
                    ...
                    for result in rec_history:
                        ...
                        text = f"{entry}\n<HEURL>https://exchange.xforce.ibmcloud.com/ip/{addr}</HEURL>"
                        e = HawkEyeEvent(malicious_type, text, self.__name__, event)
                        self.notifyListeners(e)
                        e = HawkEyeEvent(blacklist_type, text, self.__name__, event)
                        self.notifyListeners(e)
            ...
            if rec:
                ...
                if len(rec_malware) > 0:
                    ...
                    e = HawkEyeEvent("RAW_RIR_DATA", str(rec_malware), self.__name__, event)
                    self.notifyListeners(e)
        ...
        if eventName in ["IP_ADDRESS", "IPV6_ADDRESS"]:
            ...
            e = HawkEyeEvent("RAW_RIR_DATA", str(records), self.__name__, event)
            self.notifyListeners(e)
            ...
            for rec in records:
                ...
                if rec['recordType'] == "A":
                    ...
                    if self.opts['verify']:
                        if not self.he.validateIP(host, eventData):
                    ...
                    e = HawkEyeEvent("CO_HOSTED_SITE", host, self.__name__, event)
                    self.notifyListeners(e)
                    self.cohostcount += 1

# End of hep_xforce class
