...
class hep_c99(HawkEyePlugin):
    ...
    def emitHostname(self, data, event):
        if not self.he.validHost(data, self.opts['_internettlds']):
            return
        ...
        if self.getTarget().matches(data):
            ...
            self.notifyListeners(evt)
            if self.he.isDomain(data, self.opts['_internettlds']):
                evt = HawkEyeEvent('DOMAIN_NAME', data, self.__name__, event)
                self.notifyListeners(evt)
            return

        if self.cohostcount < self.opts['maxcohost']:
            if self.opts["verify"] and not self.he.validateIP(data, event.data):
            ...
            if self.cohostcount < self.opts["maxcohost"]:
                evt = HawkEyeEvent("CO_HOSTED_SITE", data, self.__name__, event)
                self.notifyListeners(evt)
                self.cohostcount += 1
    ...
    def handleEvent(self, event):
        ...
        if self.opts["api_key"] == "":
            self.error("You enabled hep_c99, but did not set an API key!")
            self.errorState = True
            return
