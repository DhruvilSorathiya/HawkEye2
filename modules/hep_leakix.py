...
class hep_leakix(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "":
            self.debug("You enabled hep_leakix but did not set an API key, results are limited")
        ...
        if eventName in ["IP_ADDRESS", "DOMAIN_NAME"]:
            ...
            if services:
                for service in services:
                    ...
                    if hostname and eventName == "DOMAIN_NAME" and self.getTarget().matches(hostname) and hostname not in hosts:
                        if self.opts["verify"] and not self.he.resolveHost(hostname) and not self.he.resolveHost6(hostname):
                            self.debug(f"Host {hostname} could not be resolved")
                            evt = HawkEyeEvent("INTERNET_NAME_UNRESOLVED", hostname, self.__name__, event)
                        else:
                            evt = HawkEyeEvent("INTERNET_NAME", hostname, self.__name__, event)
                        self.notifyListeners(evt)
                        src = evt
                        hosts.append(hostname)
                    ip = service.get('ip')
                    if ip and eventName != "IP_ADDRESS" and self.he.validIP(ip) and ip not in ips:
                        evt = HawkEyeEvent("IP_ADDRESS", ip, self.__name__, src)
                        self.notifyListeners(evt)
                        ips.append(ip)
                        ipevt = evt
                    port = service.get('port')
                    if port and ip + ":" + port not in ports:
                        evt = HawkEyeEvent("TCP_PORT_OPEN", ip + ':' + port, self.__name__, src)
                        self.notifyListeners(evt)
                        ...
                        if servers:
                            for server in servers:
                                if server and server not in banners:
                                    evt = HawkEyeEvent('WEBSERVER_BANNER', server, self.__name__, src)
                                    self.notifyListeners(evt)
                    ...
                    if geoip:
                        ...
                        if location:
                            if ip and self.he.validIP(ip) and ipevt:
                                # GEOINFO should be linked to an IP_ADDRESS
                                evt = HawkEyeEvent("GEOINFO", location, self.__name__, ipevt)
                                self.notifyListeners(evt)
                    ...
                    if software:
                        ...
                        if software_version and software_version not in softwares:
                            evt = HawkEyeEvent("SOFTWARE_USED", software_version, self.__name__, src)
                            self.notifyListeners(evt)
                            softwares.append(software_version)

                        os = software.get('os')
                        if os and os not in oses:
                            evt = HawkEyeEvent('OPERATING_SYSTEM', os, self.__name__, src)
                            self.notifyListeners(evt)
                            oses.append(os)
            ...
            if leaks:
                for leak in leaks:
                    ...
                    if leak_protocol == "web" and hostname and not self.he.validIP(hostname) and not self.getTarget().matches(hostname):
                        continue
                    leak_data = leak.get('data')
                    if leak_data:
                        evt = HawkEyeEvent("LEAKSITE_CONTENT", leak_data, self.__name__, event)
                        self.notifyListeners(evt)

# End of hep_leakix class
