...
class hep_torexits(HawkEyePlugin):
    ...
    def retrieveExitNodes(self):
        exit_addresses = self.he.cacheGet('torexitnodes', self.opts.get('cacheperiod', 1))
        ...
        self.he.cachePut("torexitnodes", res['content'])
        ...
        for relay in relays:
            ...
            if exit_addresses:
                for ip in exit_addresses:
                    if self.he.validIP(ip) or self.he.validIP6(ip):
                        ips.append(ip)
    ...
    def handleEvent(self, event):
        ...
        for addr in addrs:
            ...
            evt = HawkEyeEvent("TOR_EXIT_NODE", addr, self.__name__, pevent)
            self.notifyListeners(evt)

# End of hep_torexits class
