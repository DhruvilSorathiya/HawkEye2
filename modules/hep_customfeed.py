...
class hep_customfeed(HawkEyePlugin):
    ...
    def resourceList(self, replaceme_id, target, targetType):
        ...
        if targetType == "domain":
            targetDom = self.he.hostDomain(target, self.opts['_internettlds'])
            if not targetDom:
                return None
        ...
        for check in list(malchecks.keys()):
            ...
            if replaceme_id == cid:
                data = dict()
                data['content'] = self.he.cacheGet("hemal_" + cid, self.opts.get('cacheperiod', 0))
                if data['content'] is None:
                    data = self.he.fetchUrl(url, timeout=self.opts['_fetchtimeout'], useragent=self.opts['_useragent'])
                    if data['content'] is None:
                        self.error("Unable to fetch " + url)
                        return None
                    self.he.cachePut("hemal_" + cid, data['content'])
    ...
    def handleEvent(self, event):
        ...
        if self.opts['url'] == "":
            self.error("You enabled hep_customfeed but defined no custom feed URL!")
