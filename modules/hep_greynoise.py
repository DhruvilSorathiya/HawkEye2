...
class hep_greynoise(HawkEyePlugin):
    ...
    def queryIP(self, qry, qry_type):
        ...
        if qry_type == "ip":
            ...
            res = {}
            ip_response = self.he.fetchUrl(
                gn_context_url + qry,
                timeout=self.opts["_fetchtimeout"],
                useragent="greynoise-hawkeye-v1.2.0",
                headers=headers,
            )
            ...
            query_response = self.he.fetchUrl(
                gn_gnql_url + qry,
                timeout=self.opts["_fetchtimeout"],
                useragent="greynoise-hawkeye-v1.1.0",
                headers=headers,
            )
    ...
    def handleEvent(self, event):
        ...
        if self.opts["api_key"] == "":
            self.error("You enabled hep_greynoise but did not set an API key!")
            self.errorState = True
            return
            ...
            for rec in ret["data"]:
                if rec.get("seen", None):
                    ...
                    if rec.get("metadata") and eventName == "IP_ADDRESS":
                        ...
                        if met.get("country", "unknown") != "unknown":
                            ...
                            if met.get("city"):
                                loc = met.get("city") + ", "
                            loc += met.get("country")
                            e = HawkEyeEvent("GEOINFO", loc, self.__name__, event)
                            self.notifyListeners(e)
                        if met.get("asn", "unknown") != "unknown":
                            asn = met.get("asn").replace("AS", "")
                            e = HawkEyeEvent("BGP_AS_MEMBER", asn, self.__name__, event)
                            self.notifyListeners(e)
                        if met.get("organization", "unknown") != "unknown":
                            e = HawkEyeEvent("COMPANY_NAME", met.get("organization"), self.__name__, event)
                            self.notifyListeners(e)
                        if met.get("os", "unknown") != "unknown":
                            e = HawkEyeEvent("OPERATING_SYSTEM", met.get("os"), self.__name__, event)
                            self.notifyListeners(e)
                        e = HawkEyeEvent("RAW_RIR_DATA", str(rec), self.__name__, event)
                        self.notifyListeners(e)
                    ...
                    if rec.get("classification"):
                        ...
                        descr += "\n<HEURL>https://viz.greynoise.io/ip/" + rec.get("ip") + "</HEURL>"
                        e = HawkEyeEvent(evtType, descr, self.__name__, event)
                        self.notifyListeners(e)
            ...
            if ret.get("seen", None):
                ...
                if ret.get("metadata") and eventName == "IP_ADDRESS":
                    ...
                    if met.get("os", "unknown") != "unknown":
                        e = HawkEyeEvent("OPERATING_SYSTEM", met.get("os"), self.__name__, event)
                        self.notifyListeners(e)
                    e = HawkEyeEvent("RAW_RIR_DATA", str(ret), self.__name__, event)
                    self.notifyListeners(e)
                ...
                if ret.get("classification"):
                    ...
                    descr += "\n<HEURL>https://viz.greynoise.io/ip/" + ret.get("ip") + "</HEURL>"
                    e = HawkEyeEvent(evtType, descr, self.__name__, event)
                    self.notifyListeners(e)

# End of hep_greynoise class
