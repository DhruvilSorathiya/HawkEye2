...
class hep_template(HawkEyePlugin):
    ...
    meta = {
        ...
        'summary': "This is an example module to help developers create their own HawkEye modules.",
        ...
        # that purley parse data from other modules (e.g. hep_email), this may be omitted.
        'dataSource': {
    ...
    def setup(self, hec, userOpts=dict()):
        ...
        # instead since on HawkEye HX, different mechanisms are used to persist
        ...
        # utilised in HawkEye HX and not the open source version.
    ...
    # For a list of all events, check hawkeye/db.py.
    ...
    def query(self, qry):
        ...
        # from global config), and the user agent is HawkEye so that the
        ...
        # - fatal(message) if it's a horrible thing and should kill HawkEye completely
    ...
    def handleEvent(self, event):
        # The three most used fields in HawkEyeEvent are:
        # event.eventType - the event type, e.g. INTERNET_NAME, IP_ADDRESS, etc.
        # event.module - the name of the module that generated the event, e.g. hep_dnsresolve
        ...
        for addr in qrylist:
            ...
            if eventName == 'NETBLOCK_OWNER':
                ...
                # to process and is a fundamental part of the HawkEye architecture.
                ...
                # event of this event. This enables HawkEye to link events so users
                # can see what events generated other events, seeing a full chain of
                # discovery from their target to the data returned here.
                pevent = HawkEyeEvent("IP_ADDRESS", addr, self.__name__, event)
                ...
                pevent = HawkEyeEvent("AFFILIATE_IPADDR", addr, self.__name__, event)
                self.notifyListeners(pevent)
            else:
            ...
            evt = HawkEyeEvent("RAW_RIR_DATA", str(rec), self.__name__, pevent)
            ...
            # itself! hep_email is a good example, since it is purely looking
            ...
            os = rec.get('os')
            if os:
                evt = HawkEyeEvent("OPERATING_SYSTEM", f"{os} ({addr})", self.__name__, pevent)
                self.notifyListeners(evt)

# End of hep_template class
