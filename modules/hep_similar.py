...
# -------------------------------------------------------------------------------
# Name:         hep_similar
# Purpose:      HawkEye plug-in for identifying domains that look similar
...
class hep_similar(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if self.opts['skipwildcards'] and self.he.checkDnsWildcard(tld[1:]):
            return
            ...
            try:
                for domain in [f"{d}{tld}", f"www.{d}{tld}"]:
                    if self.he.resolveHost(domain) or self.he.resolveHost6(domain):
                        ...
                        evt = HawkEyeEvent("SIMILARDOMAIN", f"{d}{tld}", self.__name__, event)
                        self.notifyListeners(evt)
                        break
            except Exception:
                continue

# End of hep_similar class
