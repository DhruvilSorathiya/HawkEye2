...
class hep_jsonwhoiscom(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "":
            self.error("You enabled hep_jsonwhoiscom but did not set an API key!")
            self.errorState = True
            return
        ...
        for contact in contacts:
            ...
            if email:
                if HawkEyeHelpers.validEmail(email):
                    emails.append(email)
            ...
            country = HawkEyeHelpers.countryNameFromCountryCode(contact.get('country_code'))
        ...
        for email in set(emails):
            ...
            if self.getTarget().matches(mail_domain, includeChildren=True):
                if email.split("@")[0] in self.opts['_genericusers'].split(","):
                    ...
                    evttype = "EMAILADDR"
                evt = HawkEyeEvent(evttype, email, self.__name__, event)
                self.notifyListeners(evt)
            else:
                evt = HawkEyeEvent("AFFILIATE_EMAILADDR", email, self.__name__, event)
                self.notifyListeners(evt)

        if eventName in ["DOMAIN_NAME"]:
            raw = res.get('raw')
            if raw:
                evt = HawkEyeEvent("DOMAIN_WHOIS", raw, self.__name__, event)
                self.notifyListeners(evt)
            ...
            if registrar:
                ...
                if registrar_name:
                    evt = HawkEyeEvent("DOMAIN_REGISTRAR", registrar_name, self.__name__, event)
                    self.notifyListeners(evt)

            for dns_provider in set(dns_providers):
                evt = HawkEyeEvent("PROVIDER_DNS", dns_provider, self.__name__, event)
                self.notifyListeners(evt)

            for name in set(names):
                evt = HawkEyeEvent("RAW_RIR_DATA", f"Possible full name {name}", self.__name__, event)
                self.notifyListeners(evt)

            for phone in set(phones):
                evt = HawkEyeEvent("PHONE_NUMBER", phone, self.__name__, event)
                self.notifyListeners(evt)
        ...
        if eventName in ["AFFILIATE_DOMAIN_NAME"]:
            raw = res.get('raw')
            if raw:
                evt = HawkEyeEvent("AFFILIATE_DOMAIN_WHOIS", raw, self.__name__, event)
                self.notifyListeners(evt)

            available = res.get('available?')
            if available:
                evt = HawkEyeEvent("AFFILIATE_DOMAIN_UNREGISTERED", eventData, self.__name__, event)
                self.notifyListeners(evt)

# End of hep_jsonwhoiscom class
