# -*- coding: utf-8 -*-
# -------------------------------------------------------------------------------
# Name:        hep_tool_testsslsh
# Purpose:     HawkEye plug-in for using the testssl.sh tool.
...
from hawkeye import HawkEyePlugin, HawkEyeEvent, HawkEyeHelpers


class hep_tool_testsslsh(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if srcModuleName == "hep_tool_testsslsh":
            self.debug("Skipping event from myself.")
            return

        if not self.opts['testsslsh_path']:
            self.error("You enabled hep_tool_testsslsh but did not set a path to the tool!")
        ...
        if not HawkEyeHelpers.sanitiseInput(eventData, extra=['/']):
        ...
        for target in targets:
            ...
            if eventName == 'NETBLOCK_OWNER':
                ...
                if generate:
                    pevent = HawkEyeEvent("IP_ADDRESS", target, self.__name__, event)
            ...
            for result in result_json:
                ...
                if 'cve' in result:
                    ...
                    evt = HawkEyeEvent("VULNERABILITY_GENERAL", f"{result['id']} ({result['finding']})", self.__name__, pevent)
                    self.notifyListeners(evt)

# End of hep_tool_testsslsh class
