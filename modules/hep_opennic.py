...
# -------------------------------------------------------------------------------
# Name:        hep_opnenic
# Purpose:     HawkEye plug-in for resolving host names on the OpenNIC
...
class hep_opennic(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        addrs = self.he.normalizeDNS(self.queryOpenNIC(eventData))
        ...
        for addr in set(addrs):
            if self.he.validIP(addr):
                if affiliate and not self.getTarget().matches(addr, includeParents=True):
                    evt = HawkEyeEvent("AFFILIATE_IPADDR", addr, self.__name__, event)
                else:
                    evt = HawkEyeEvent("IP_ADDRESS", addr, self.__name__, event)

                self.notifyListeners(evt)
            elif self.he.validIP6(addr):
                if affiliate and not self.getTarget().matches(addr, includeParents=True):
                    evt = HawkEyeEvent("AFFILIATE_IPV6_ADDRESS", addr, self.__name__, event)
                else:
                    evt = HawkEyeEvent("IPV6_ADDRESS", addr, self.__name__, event)

                self.notifyListeners(evt)
            else:
                continue

# End of hep_opennic class
