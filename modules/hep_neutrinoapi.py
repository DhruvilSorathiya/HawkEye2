# -*- coding: utf-8 -*-
# -------------------------------------------------------------------------------
# Name:        hep_neutrinoapi
# Purpose:     HawkEye plug-in to search NeutrinoAPI for IP address info,
...
class hep_neutrinoapi(HawkEyePlugin):
    ...
    def handleEvent(self, event):
        ...
        if self.opts['api_key'] == "":
            self.error("You enabled hep_neutrinoapi but did not set an API key!")
            self.errorState = True
            return

        if self.opts['user_id'] == "":
            self.error("You enabled hep_neutrinoapi but did not set a user ID!")
            self.errorState = True
            return
        ...
        if eventName in ['IP_ADDRESS', 'IPV6_ADDRESS']:
            ...
            if data is None:
                ...
                if data.get('is-listed'):
                    evt = HawkEyeEvent("MALICIOUS_IPADDR", f"NeutrinoAPI - IP Blocklist [{eventData}]", self.__name__, event)
                    self.notifyListeners(evt)
                    evt = HawkEyeEvent("BLACKLISTED_IPADDR", f"NeutrinoAPI - IP Blocklist [{eventData}]", self.__name__, event)
                    self.notifyListeners(evt)
                    evt = HawkEyeEvent("RAW_RIR_DATA", str(data), self.__name__, event)
                    self.notifyListeners(evt)
            ...
            if data is None:
                ...
                if data.get('is-listed'):
                    evt = HawkEyeEvent("MALICIOUS_IPADDR", f"NeutrinoAPI - Host Reputation [{eventData}]", self.__name__, event)
                    self.notifyListeners(evt)
                    evt = HawkEyeEvent("BLACKLISTED_IPADDR", f"NeutrinoAPI - Host Reputation [{eventData}]", self.__name__, event)
                    self.notifyListeners(evt)
                    evt = HawkEyeEvent("RAW_RIR_DATA", str(data), self.__name__, event)
                    self.notifyListeners(evt)

# End of hep_neutrinoapi class
