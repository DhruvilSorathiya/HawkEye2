...
class hep_urlscan(HawkEyePlugin):
    ...
    def query(self, qry):
        ...
        res = self.he.fetchUrl('https://urlscan.io/api/v1/search/?' + urllib.parse.urlencode(params),
    ...
    def handleEvent(self, event):
        ...
        evt = HawkEyeEvent('RAW_RIR_DATA', str(results), self.__name__, event)
        ...
        for res in results:
            ...
            if self.getTarget().matches(self.he.urlFQDN(url), includeParents=True):
                urls.append(url)
        ...
        for domain in set(domains):
            if self.opts['verify'] and not self.he.resolveHost(domain) and not self.he.resolveHost6(domain):
                evt = HawkEyeEvent('INTERNET_NAME_UNRESOLVED', domain, self.__name__, event)
                self.notifyListeners(evt)
            else:
                evt = HawkEyeEvent('INTERNET_NAME', domain, self.__name__, event)
                self.notifyListeners(evt)
        ...
        for asn in set(asns):
            evt = HawkEyeEvent('BGP_AS_MEMBER', asn, self.__name__, event)
            self.notifyListeners(evt)

        for server in set(servers):
            evt = HawkEyeEvent('WEBSERVER_BANNER', server, self.__name__, event)
            self.notifyListeners(evt)

# End of hep_ipinfo class
