...
from hawkeye import HawkEyeThreadPool


@pytest.mark.usefixtures
class TestHawkEyeThreadPool(unittest.TestCase):
    """
    Test HawkEye
    """
    ...
    def test_threadPool(self):
        """
        Test ThreadPool(hep, threads=10)
        """
        threads = 10
        ...
        # Example 1: using map()
        with HawkEyeThreadPool(threads) as pool:
            map_results = sorted(
                list(pool.map(
                    callback,
                    iterable,
        ...
        pool = HawkEyeThreadPool(threads)
        pool.start()
        for i in iterable2:
