...
class TestHawkEyeHelpers(unittest.TestCase):

    def test_data_path_should_return_a_string(self):
        data_path = HawkEyeHelpers.dataPath()
        self.assertIsInstance(data_path, str)

    def test_cache_path_should_return_a_string(self):
        cache_path = HawkEyeHelpers.cachePath()
        self.assertIsInstance(cache_path, str)

    def test_log_path_should_return_a_string(self):
        log_path = HawkEyeHelpers.logPath()
        self.assertIsInstance(log_path, str)

    def test_target_type(self):
        target_type = HawkEyeHelpers.targetTypeFromString("0.0.0.0")
        self.assertEqual('IP_ADDRESS', target_type)
        target_type = HawkEyeHelpers.targetTypeFromString("noreply@hawkeye.net")
        self.assertEqual('EMAILADDR', target_type)
        target_type = HawkEyeHelpers.targetTypeFromString("0.0.0.0/0")
        self.assertEqual('NETBLOCK_OWNER', target_type)
        target_type = HawkEyeHelpers.targetTypeFromString("+1234567890")
        self.assertEqual('PHONE_NUMBER', target_type)
        target_type = HawkEyeHelpers.targetTypeFromString('"Human Name"')
        self.assertEqual('HUMAN_NAME', target_type)
        target_type = HawkEyeHelpers.targetTypeFromString('"abc123"')
        self.assertEqual('USERNAME', target_type)
        target_type = HawkEyeHelpers.targetTypeFromString("1234567890")
        self.assertEqual('BGP_AS_OWNER', target_type)
        target_type = HawkEyeHelpers.targetTypeFromString("::1")
        self.assertEqual('IPV6_ADDRESS', target_type)
        target_type = HawkEyeHelpers.targetTypeFromString("hawkeye.net")
        self.assertEqual('INTERNET_NAME', target_type)
        target_type = HawkEyeHelpers.targetTypeFromString("1HesYJSP1QqcyPEjnQ9vzBL1wujruNGe7R")
        self.assertEqual('BITCOIN_ADDRESS', target_type)

    def test_target_type_invalid_seed_should_return_none(self):
        target_type = HawkEyeHelpers.targetTypeFromString(None)
        self.assertEqual(None, target_type)

        target_type = HawkEyeHelpers.targetTypeFromString("")
        self.assertEqual(None, target_type)
    ...
    def test_urlRelativeToAbsolute_argument_url_relative_path_should_return_an_absolute_path(self):
        relative_url = HawkEyeHelpers.urlRelativeToAbsolute('/somewhere/else/../../path?param=value#fragment')
    ...
    def test_url_base_dir_should_return_a_string(self):
        base_dir = HawkEyeHelpers.urlBaseDir('http://localhost.local/path?param=value#fragment')
    ...
    def test_url_base_url_should_return_a_string(self):
        base_url = HawkEyeHelpers.urlBaseUrl('http://localhost.local/path?param=value#fragment')
    ...
    def test_dictionaryWordsFromWordlists_should_return_a_set(self):
        words = HawkEyeHelpers.dictionaryWordsFromWordlists()
    ...
    def test_dictionaryWordsFromWordlists_argument_wordlists_missing_wordlist_should_raise_IOError(self):
        with self.assertRaises(IOError):
            HawkEyeHelpers.dictionaryWordsFromWordlists(['does not exist'])

    def test_humanNamesFromWordlists_should_return_a_set(self):
        names = HawkEyeHelpers.humanNamesFromWordlists()
    ...
    def test_humanNamesFromWordlists_argument_wordlists_missing_wordlist_should_raise_IOError(self):
        with self.assertRaises(IOError):
            HawkEyeHelpers.humanNamesFromWordlists(['does not exist'])

    def test_usernamesFromWordlists_should_return_a_set(self):
        users = HawkEyeHelpers.usernamesFromWordlists()
    ...
    def test_usernamesFromWordlists_argument_wordlists_missing_wordlist_should_raise_IOError(self):
        with self.assertRaises(IOError):
            HawkEyeHelpers.usernamesFromWordlists(['does not exist'])
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                with self.assertRaises(TypeError):
                    HawkEyeHelpers.buildGraphData(invalid_type)

    def test_buildGraphData_empty_data_should_raise_ValueError(self):
        with self.assertRaises(ValueError):
            HawkEyeHelpers.buildGraphData([])
    ...
    def test_buildGraphData_should_return_a_set(self):
        graph_data = HawkEyeHelpers.buildGraphData(
            [
    ...
    def test_buildGraphGexf_should_return_bytes(self):
        gexf = HawkEyeHelpers.buildGraphGexf('test root', 'test title', [["test", "test", "test", "test", "test", "test", "test", "test", "test", "test", "test", "ENTITY", "test", "test", "test"]])
        self.assertIsInstance(gexf, bytes)

        self.assertEqual('TBD', 'TBD')

    def test_buildGraphJson_should_return_a_string(self):
        json = HawkEyeHelpers.buildGraphJson('test root', [["test", "test", "test", "test", "test", "test", "test", "test", "test", "test", "test", "ENTITY", "test", "test", "test"]])
    ...
    def test_dataParentChildToTree_invalid_data_type_should_return_TypeError(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                with self.assertRaises(TypeError):
                    HawkEyeHelpers.dataParentChildToTree(invalid_type)

    def test_dataParentChildToTree_empty_data_should_return_ValueError(self):
        with self.assertRaises(ValueError):
            HawkEyeHelpers.dataParentChildToTree(dict())

    def test_dataParentChildToTree_should_return_dict(self):
        tree = HawkEyeHelpers.dataParentChildToTree({"test": {"123": "456"}})
        self.assertIsInstance(tree, dict)

    def test_genScanInstanceId_should_return_a_string(self):
        scan_instance_id = HawkEyeHelpers.genScanInstanceId()
    ...
    def test_extractLinksFromHtml_argument_data_not_containing_links_should_return_an_empty_dict(self):
        parse_links = HawkEyeHelpers.extractLinksFromHtml('url', '<html>example html content</html>', 'domains')
    ...
    def test_extractLinksFromHtml_argument_data_containing_malformed_html_with_links_should_return_a_dict_of_urls(self):
        url = 'http://hawkeye.net/'
        parse_links = HawkEyeHelpers.extractLinksFromHtml(
            url,
            '<!DOCTYPE html><html lang="en-US"><meta charset="UTF-8" /><link rel="pingback" href="http://hawkeye.net/xmlrpc.php">example html content<unclosed tag><a href="http://hawkeye.net/path"></a><a href="/relative-path"></a></html>',
            'domains'
        )
        self.assertIsInstance(parse_links, dict)
        ...
        self.assertEqual(
            parse_links,
            {
                'http://hawkeye.net/xmlrpc.php': {'source': url, 'original': 'http://hawkeye.net/xmlrpc.php'},
                'http://hawkeye.net/path': {'source': url, 'original': 'http://hawkeye.net/path'},
                'http://hawkeye.net/relative-path': {'source': url, 'original': '/relative-path'},
            }
        )
    ...
    def test_extractLinksFromHtml_invalid_url_should_raise_TypeError(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                with self.assertRaises(TypeError):
                    HawkEyeHelpers.extractLinksFromHtml(invalid_type, 'example html content', 'domains')
    ...
    def test_extractLinksFromHtml_invalid_data_should_raise_TypeError(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                with self.assertRaises(TypeError):
                    HawkEyeHelpers.extractLinksFromHtml("", invalid_type, 'domains')

    def test_extractLinksFromHtml_invalid_domains_should_return_a_dict(self):
        invalid_types = [None, "", bytes(), list(), dict()]
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                parse_links = HawkEyeHelpers.extractLinksFromHtml('url', 'example html content', invalid_type)
                self.assertIsInstance(parse_links, dict)
    ...
    def test_extractHashesFromText_should_return_a_list(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                hashes = HawkEyeHelpers.extractHashesFromText(invalid_type)
    ...
    def test_extractHashesFromText_argument_data_should_return_hahes(self):
        ...
        hashes = HawkEyeHelpers.extractHashesFromText(f"hawkeye{md5_hash}hawkeye{sha1_hash}hawkeye{sha256_hash}hawkeye{sha512_hash}hawkeye")
    ...
    def test_valid_email_should_return_a_boolean(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                valid_email = HawkEyeHelpers.validEmail(invalid_type)
                self.assertIsInstance(valid_email, bool)
                self.assertFalse(valid_email)

        valid_email = HawkEyeHelpers.validEmail('%@localhost.local')
        self.assertIsInstance(valid_email, bool)
        self.assertFalse(valid_email)
        ...
        valid_email = HawkEyeHelpers.validEmail('root@localhost.local\n.com')
        self.assertIsInstance(valid_email, bool)
        self.assertFalse(valid_email)

        valid_email = HawkEyeHelpers.validEmail('root@localhost')
        self.assertIsInstance(valid_email, bool)
        self.assertFalse(valid_email)
    ...
    def test_validPhoneNumber_should_return_a_boolean(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                valid_phone = HawkEyeHelpers.validPhoneNumber(invalid_type)
                self.assertIsInstance(valid_phone, bool)
                self.assertFalse(valid_phone)

        valid_phone = HawkEyeHelpers.validPhoneNumber('+1234567890')
        self.assertIsInstance(valid_phone, bool)
        self.assertFalse(valid_phone)

        valid_phone = HawkEyeHelpers.validPhoneNumber('+12345678901234567890')
        self.assertIsInstance(valid_phone, bool)
        self.assertFalse(valid_phone)

        valid_phone = HawkEyeHelpers.validPhoneNumber('+12345678901')
        self.assertIsInstance(valid_phone, bool)
    ...
    def test_validLEI_should_return_a_boolean(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                valid_phone = HawkEyeHelpers.validLEI(invalid_type)
                self.assertIsInstance(valid_phone, bool)
                self.assertFalse(valid_phone)

        valid_lei = HawkEyeHelpers.validLEI('7ZW8QJWVPR4P1J1KQYZZ')
        self.assertIsInstance(valid_lei, bool)
        self.assertFalse(valid_lei)

        valid_lei = HawkEyeHelpers.validLEI('7ZW8QJWVPR4P1J1KQY45')
        self.assertIsInstance(valid_lei, bool)
        self.assertTrue(valid_lei)

    def test_countryCodes_should_return_a_dict(self):
        country_code_dict = HawkEyeHelpers.countryCodes()
    ...
    def test_countryNameFromCountryCode_argument_countryCode_should_return_country_as_a_string(self):
        country_name = HawkEyeHelpers.countryNameFromCountryCode('US')
    ...
    def test_countryNameFromTld_argument_tld_should_return_country_as_a_string(self):
        ...
        for tld in tlds:
            with self.subTest(tld=tld):
                country_name = HawkEyeHelpers.countryNameFromTld(tld)
                self.assertIsInstance(country_name, str)
                self.assertEqual(country_name, "United States")

    def test_extractEmailsFromText_should_return_list_of_emails_from_string(self):
        emails = HawkEyeHelpers.extractEmailsFromText("<html><body><p>From:noreply@hawkeye.net</p><p>Subject:Hello user@hawkeye.net, here's some text</p></body></html>")
        self.assertIsInstance(emails, list)
        self.assertIn('noreply@hawkeye.net', emails)
        self.assertIn('user@hawkeye.net', emails)

    def test_extractEmailsFromText_invalid_data_should_return_list(self):
        invalid_types = [None, "", bytes(), list(), dict()]
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                emails = HawkEyeHelpers.extractEmailsFromText(invalid_type)
                self.assertIsInstance(emails, list)
    ...
    def test_extractPgpKeysFromText_should_return_list_of_pgp_keys_from_string(self):
        ...
        keys = HawkEyeHelpers.extractPgpKeysFromText(f"<html><body><p>sample{key1}sample</p><p>sample{key2}sample</p></body></html>")
    ...
    def test_extractIbansFromText_should_return_a_list(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                ibans = HawkEyeHelpers.extractIbansFromText(invalid_type)
        ...
        for iban in ibans:
            with self.subTest(iban=iban):
                extract_ibans = HawkEyeHelpers.extractIbansFromText(iban)
    ...
    def test_extractCreditCardsFromText_should_return_a_list(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                cards = HawkEyeHelpers.extractCreditCardsFromText(invalid_type)
                self.assertIsInstance(cards, list)

        cards = HawkEyeHelpers.extractCreditCardsFromText("hawkeye4111 1111 1111 1111hawkeye")
        self.assertIsInstance(cards, list)
        self.assertEqual(["4111111111111111"], cards)

    def test_sslDerToPem_should_return_a_certificate_as_a_string(self):
        pem = HawkEyeHelpers.sslDerToPem(
    ...
    def test_sslDerToPem_invalid_cert_should_raise_TypeError(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type), self.assertRaises(TypeError):
                HawkEyeHelpers.sslDerToPem(invalid_type)

    def test_extractUrlsFromText_should_extract_urls_from_string(self):
        invalid_types = [None, "", bytes(), list(), dict()]
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                cards = HawkEyeHelpers.extractUrlsFromText(invalid_type)
                self.assertIsInstance(cards, list)

        urls = HawkEyeHelpers.extractUrlsFromText("abchttps://example.hawkeye.net/path\rabchttp://example.hawkeye.net:1337/path\rabc")
        self.assertIsInstance(urls, list)
        self.assertIn("https://example.hawkeye.net/path", urls)
        self.assertIn("http://example.hawkeye.net:1337/path", urls)

    def test_extractUrlsFromRobotsTxt_should_return_list(self):
        invalid_types = [None, "", bytes(), list(), dict()]
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                robots_txt = HawkEyeHelpers.extractUrlsFromRobotsTxt(invalid_type)
                self.assertIsInstance(robots_txt, list)

        robots_txt = HawkEyeHelpers.extractUrlsFromRobotsTxt("disallow:")
        self.assertIsInstance(robots_txt, list)
        self.assertFalse(robots_txt)

        robots_txt = HawkEyeHelpers.extractUrlsFromRobotsTxt("disallow: /disallowed/path\n")
        self.assertIsInstance(robots_txt, list)
        self.assertIn("/disallowed/path", robots_txt)

    def test_sanitise_input(self):
        safe = HawkEyeHelpers.sanitiseInput("example-string")
        ...
        safe = HawkEyeHelpers.sanitiseInput("12")
        self.assertIsInstance(safe, bool)
        self.assertFalse(safe)
