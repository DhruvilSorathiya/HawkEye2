...
class TestHawkEyeWebUiRoutes(helper.CPWebCase):
    @staticmethod
    def setup_server():
        default_config = {
            '_debug': False,  # Debug
        ...
        default_config['__modules__'] = HawkEyeHelpers.loadModulesAsDict(mod_dir, ['hep_template.py'])
        ...
        cherrypy.tree.mount(HawkEyeWebUi(default_web_config, default_config), script_name=default_web_config.get('root'), config=conf)
    ...
    def test_static_returns_200(self):
        self.getPage("/static/img/hawkeye-header.png")
    ...
    def test_optsexport(self):
        ...
        self.assertStatus('200 OK')
        self.assertHeader("Content-Disposition", "attachment; filename=\"HawkEye.cfg\"")
    ...
    def test_startscan_unrecognized_scan_target_returns_error(self):
        ...
        self.assertStatus('200 OK')
        self.assertInBody('Invalid target type. Could not recognize it as a target HawkEye supports.')

    def test_startscan_invalid_modules_returns_error(self):
        self.getPage("/startscan?scanname=example-scan&scantarget=hawkeye.net&modulelist=&typelist=&usecase=")
        self.assertStatus('200 OK')
        self.assertInBody('Invalid request: no modules specified for scan.')

    def test_startscan_invalid_typelist_returns_error(self):
        self.getPage("/startscan?scanname=example-scan&scantarget=hawkeye.net&modulelist=&typelist=doesnotexist&usecase=")
        self.assertStatus('200 OK')
        self.assertInBody('Invalid request: no modules specified for scan.')

    def test_startscan_should_start_a_scan(self):
        self.getPage("/startscan?scanname=hawkeye.net&scantarget=hawkeye.net&modulelist=doesnotexist&typelist=doesnotexist&usecase=doesnotexist")
